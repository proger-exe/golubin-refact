...
WEBAPP_PORT = 5000
# bot constants
PAYING_BOT_TOKEN = '5023421808:AAHvhN8e4lxPI_npm4kxVpLb5u078-x4maY' if not config.TESTING else \
...
USERS_THAT_ALLOWED_TO_DELETE_MESSAGES = (EUGENIY_ID, 476806184, 454528448, 829894707, 5035325356, 1787839420,
    1236978331, 5091768057, 486877869, 1617121573) if not config.TESTING else (DEVELOPER_ID,)
...
Выберите период:
'''
APPLICATIONS_FROM_USERS_DIR = './new messages/from_clients' if not config.TESTING else './temp new messages/from_clients'
...
# yoomoney api
CLIENT_TOKEN =  '4100117427295843.'
...
CATEGORIES_WITHOUT_SALES = (config.TARGET_MODE, config.CONTEXT_MODE, config.SMM_MODE, config.JURISPRUDENCE_MODE, config.SITES_MODE, config.TUTOR_MODE)
...
LAST_HANDLED_BOTS_UPDATES = 'last_handled_bot_updates.json' if not config.TESTING else 'tests/last_handled_bot_updates.json'

#bot senders
BOT_SENDER_POLLING_TIMEOUT = 1
bot_tokens = {
    config.TARGET_MODE: '5096903131:AAHotW2Wbzon6OhGooF4q4cK4LbVvkdOFm0' if not config.TESTING else '5017859009:AAFlfrZGpgZFjmlPItWi6ODr_e0HVNihITA',
    config.SMM_MODE: '5302119349:AAFY3EKhgyAkd6CoQPTrqYWTn7wWSMR5uCA' if not config.TESTING else '',
    config.COPYRIGHT_MODE: '5371959061:AAEpPUEXTSGAMXVuFWs3rNCokUC2cr4iHpQ' if not config.TESTING else '',
    config.SITES_MODE: '5939090257:AAELmjm9E8Gzqr_ovBdKE0nEfDAA1u922rQ' if not config.TESTING else '',
    config.CHAT_BOTS_MODE: '5314134561:AAFYUFMiCi7ZJA-VkYBj8Zfb-X5B1_VjaHI' if not config.TESTING else '',
    config.SEO_MODE: '5345468905:AAE5cP_p6DoJl1BMMi1xmliL95mk7dbqGCA' if not config.TESTING else '',
    config.DESIGN_MODE: '5233479598:AAG27SLV5hA6yYe3iU1v-JAaljmOHuLdgA0' if not config.TESTING else '',
    config.CONTEXT_MODE: '5190691774:AAGYtpL7f61I_Jsh8_KLs84ReyJslNTZNno' if not config.TESTING else '',
    config.PRODUSER_MODE: '5328569451:AAFQu8KmhS47_MjFz-mcDO9n_yPjRJQ64kY' if not config.TESTING else '',
    #MARKETING_MODE: '5359352487:AAG_eA6wul9OSQN9cY3C5Cyd41k03OLSdBE' if not data.TESTING else '',
    config.AVITOLOG_MODE: '5285936482:AAFep2-hYpHObVLV_OCnkIagCxljAu9cLjo' if not config.TESTING else '',
    config.JURISPRUDENCE_MODE: '5433997522:AAENpja1NWkZY3ChqIHgnj4FTwbNRhk7Qtw' if not config.TESTING else '',
    config.PSYCHOLOGY_MODE: '6262966675:AAHuYSKvuZuf1TPTA6U9-NA0Feks9JX12_I' if not config.TESTING else '',
    config.SURGERY_MODE: '6126851961:AAGS54TjvwMbwQ3aIr3pbtPvfCob_XuXR7I' if not config.TESTING else '5165724922:AAGpFdffemYGZaDJunn5jcw-wqxmI9AFA9M',
    config.INTERIOR_MODE: '6182503785:AAHmssoZ_yJbtQhuQtj57YhMB1VT4--WncE' if not config.TESTING else '5394143236:AAGhzfpkAVCj6RXuEMFUZ6Hg3qs59wyEtOg',
    config.TUTOR_MODE: '6368697157:AAF7yFkaOVAsqhfFE80GVdmvOT672AF5ltU' if not config.TESTING else '',
    config.TUTOR_ENGLISH_MODE: '6096717890:AAFVof9jQO9IrpeD-bj1UDNP6h_TNxDQJL4' if not config.TESTING else '6096717890:AAFVof9jQO9IrpeD-bj1UDNP6h_TNxDQJL4',
    config.ASSISTANT_MODE: '6365924046:AAH5RCppD6XKNSuvx8xFyH6YQ1o_SK1sTI8' if not config.TESTING else '',
    config.MARKETPLACES_MODE: '6285785063:AAFgDphuAE88ZAYdtzISNuwIgEnDIRlzWc0' if not config.TESTING else '',
    config.BOTS_MODE: '6592499623:AAHM3Cak_HcQR_KwuNO5HW7HNlqml0TG-cw' if not config.TESTING else '',
    config.SALES_MODE: '6552420841:AAHYMojHMBsmuSDs5I9izMpg-rliRtBJUIQ' if not config.TESTING else '',
    config.INVESTMENTS_MODE: '6400491190:AAFMlCTnYY5dKTk_u1KsQn8uu-M0cRNQRaI' if not config.TESTING else '',
    config.ACCOUNTANT_MODE: '6546669411:AAGiFx4JrTp7DxZfzpYX8UEtGkIR6M28LNo' if not config.TESTING else '',
    config.NUTRITION_MODE: '6438000419:AAEKsPyp_W7W7lYsShZSiDWEKHa-SWfUSFc' if not config.TESTING else '',
    config.MARKING_MODE: '6401381238:AAF9qNCOngEqD8NYfgpwqsP6oZ06qUBvgh8' if not config.TESTING else '',
    config.CARGO_MODE: '6339760906:AAEbIziByIs8dFalIWXuapl7GNB9XloSk6o' if not config.TESTING else '',
    config.FULLFILLMENT_MODE: '6515209009:AAFgx0p3rq5CmbuFark4rYbI8oqmSId3zfU' if not config.TESTING else '',
    config.ANALYTICS_MODE: '6483567603:AAG6CjVjM7dUYnKf6WKL2qszRhzqQI547s8' if not config.TESTING else '',
    config.BEAUTYDUBAI_MODE: '6471148189:AAHrq0Bk7GHL66yfM_FnBOIdIjqFSHZDNcc' if not config.TESTING else '',
    config.METODOLOGY_MODE: '6099323781:AAFPTIKso030MJLWnrG8eGiFnM9R_PMJtVM' if not config.TESTING else '',
    config.CROPS_MODE: '6671170791:AAFW-l4_spKVdFlqmO7AcrtPOs9vzql0klY' if not config.TESTING else '',
    config.CRYPTO_MODE: '6664614378:AAFsISLjaggjt3RnwZ2yeq8d7MJFohbMTgI' if not config.TESTING else '',
    config.CERTIFICATION_MODE: '6368407592:AAHVOhToqOny0tGJOwFMWD_1Vo0c_kPqllk' if not config.TESTING else '',
    config.ENGINEER_MODE: '6469650695:AAGK_KBKLw_4N3a_tpq25Fp3L5JayQKDP8k' if not config.TESTING else '', 
    config.COUCH_MODE: '6484118843:AAE8fuFpGiljQp4h1KHs2o64Z1Z89iZsTpI' if not config.TESTING else '',
    config.MANAGER_MODE: '6901781390:AAEE3dkXVL4Z_eF0YRi8RzRVDW3UrSEKZrU' if not config.TESTING else '',
    config.PHOTOGRAPHER_MODE: '6854628650:AAEtC2Wut8PS0_5xpbZj2EMbyTAyOQnalbk' if not config.TESTING else '',
    config.PROPERTY_MODE: '6582621869:AAFwDJFIRHlWh79jylc5DpEkH4GH2kJQgI8' if not config.TESTING else '',
    config.SUGARING_DEPILATION_MODE: '6665765680:AAGlRQsx-eZoRcBnVb2QLVgc4mx7eNswzM8' if not config.TESTING else '',
    config.ANIMATOR_MODE: '6981435860:AAGVkWIJ4PpcyhthNg0vtHSNn2AwrV-uQqE' if not config.TESTING else '',
    config.TRANSPORTATION_MODE: '6871043011:AAHXfiCSenuMUrFYC9uyn5ODn04WnAVwGns' if not config.TESTING else '',
    config.GERMAN_MODE: '6972699690:AAEtLJ8sGQPDwoZRvQ9I3_oI_KQEyKBUiIQ' if not config.TESTING else '',
    config.DENT_MODE: '6965522067:AAFoyqs9oWwbwTlEiIb8fJ8Vkn0tv1WWPp0' if not config.TESTING else '',
    config.REVIEWS_MODE: '6951500758:AAGm2OUuO4Qd8uJm3THAwpBR17gCAGYZY8E' if not config.TESTING else '',
    config.VIDEOGRAPH_DUBAI_MODE: '6925307897:AAGuBhYzEwo3G2_U5Bc8hMhD8y7xSizR5j4' if not config.TESTING else '',
    config.HRIT_MODE: '6785487249:AAFQom2M3rsKNsPapmiWAe95yLs4aCssjlU' if not config.TESTING else '',
    config.PR_MODE: '6866862068:AAGYuQfRrhg8jkLHTH9pjBQ7FcTA4l_pFFY' if not config.TESTING else '',
    config.VOCALS_MODE: '6955720090:AAHdbd6Ef0M5eptWDXZKoSyi8O8PgBumCaI' if not config.TESTING else '',
    config.TAILORING_MODE: '6582789621:AAEJypLOe2Vhr08OniijglRhj3BBH27xkJQ' if not config.TESTING else '',
    config.TRANSLATE_MODE: '6657375465:AAE6grc_HrSxXYQa9dhRbmpCyJW3uqvqa1g' if not config.TESTING else '',
    config.CUSTOMS_MODE: '6502941955:AAH4fCFayZnVQomy2T-1v2W7LtvA6Zd0NH8' if not config.TESTING else '',
    config.FURNITURE_MODE: '6808181937:AAHvRuJJr8_lVeFd88IbnQ8MSdL0Z6VLGAw' if not config.TESTING else '',
    config.MUSIC_MODE: '6969167896:AAGua1_hXlV5b-xMbr6F4kwBK7kQr4C1viw' if not config.TESTING else '',
    config.TECH_FIXING_MODE: '6551553767:AAE5cs0O4LvJCyxstybJRJqVu8tVvcnK5LQ' if not config.TESTING else '',
    #GEN_WROKERS_MODE: '6409080274:AAGthreEctjdfRD5EsskxEf-1Ymz-7AIz0I' if not data.TESTING else '',
    config.HR_MODE: '6807907768:AAHCbdsIMTiF7XuTG1Jvb16Xk7Gu2yj9lr8' if not config.TESTING else '',
    config.CLEANNING_MODE: '6730954087:AAHAbfH_7SWj6lV8Djr3vH-K8V3vipBfaiI' if not config.TESTING else '',
    config.BUSINESS_SALE_MODE: '6885773906:AAFlIJ4O2o1tgIAsgqlt7dgNNjD8Yxa3s7k' if not config.TESTING else '',
    config.REELSMAKER_MODE: '6958769078:AAEEQ9PsDeL2u_7l06otRM6h47y-YpxYXzM' if not config.TESTING else ''
}
bot_names = {
    config.TARGET_MODE: 'golubin_target_bot' if not config.TESTING else 'golubin_target_test_bot',
    config.SMM_MODE: 'golubin_smm_bot' if not config.TESTING else 'golubin_smm_test_bot',
    config.COPYRIGHT_MODE: 'golubin_copyright_bot' if not config.TESTING else 'golubin_copyright_test_bot',
    config.SITES_MODE: 'golubin_web_bot' if not config.TESTING else 'golubin_sties_test_bot',
    config.CHAT_BOTS_MODE: 'golubin_chat_bots_bot',
    config.SEO_MODE: 'golubin_seo_bot',
    config.DESIGN_MODE: 'golubin_design_bot',
    config.CONTEXT_MODE: 'golubin_context_bot',
    config.PRODUSER_MODE: 'golubin_producer_bot',
    config.MARKETING_MODE: 'golubin_marketing_bot',
    config.AVITOLOG_MODE: 'golubin_avitolog_bot',
    config.JURISPRUDENCE_MODE: 'golubin_jurisprudence_bot',
    config.PSYCHOLOGY_MODE: 'golubin_psychology_bot',
    config.SURGERY_MODE: 'golubin_surgery_bot',
    config.INTERIOR_MODE: 'golubin_interior_design_bot',
    config.TUTOR_MODE: 'golubin_tutor_bot',
    config.TUTOR_ENGLISH_MODE: 'golubin_tutor_english_bot',
    config.ASSISTANT_MODE: 'golubin_assistant_bot',
    config.MARKETPLACES_MODE: 'golubin_marketplaces_bot',
    config.BOTS_MODE: 'golubin_bots_bot',
    config.SALES_MODE: 'golubin_sales_bot',
    config.INVESTMENTS_MODE: 'golubin_investments_bot',
    config.ACCOUNTANT_MODE: 'golubin_accountant_bot',
    config.NUTRITION_MODE: 'golubin_nutrition_bot',
    config.MARKING_MODE: 'golubin_marking_bot',
    config.CARGO_MODE: 'golubin_cargo_bot',
    config.FULLFILLMENT_MODE: 'golubin_fullmillment_bot',
    config.ANALYTICS_MODE: 'golubin_analytics_bot',
    config.BEAUTYDUBAI_MODE: 'golubin_beautydubai_bot',
    config.METODOLOGY_MODE: 'golubin_metodology_bot',
    config.CROPS_MODE: 'golubin_crops_bot',
    config.CRYPTO_MODE: 'golubin_crypto_bot',
    config.CERTIFICATION_MODE: 'golubin_certification_bot',
    config.ENGINEER_MODE: 'golubin_engineer_bot', 
    config.COUCH_MODE: 'golubin_couch_bot',
    config.MANAGER_MODE: 'golubin_manager_bot',
    config.PHOTOGRAPHER_MODE: 'golubin_photographer_bot',
    config.PROPERTY_MODE: 'golubin_property_bot',
    config.SUGARING_DEPILATION_MODE: 'golubin_sugaring_depilation_bot',
    config.ANIMATOR_MODE: 'golubin_animator_bot',
    config.TRANSPORTATION_MODE: 'golubin_transportation_bot',
    config.GERMAN_MODE: 'golubin_deutch_bot',
    config.DENT_MODE: 'golubin_dent_bot',
    config.REVIEWS_MODE: 'golubin_reviews_bot',
    config.VIDEOGRAPH_DUBAI_MODE: 'golubin_videograph_dubai_bot',
    config.HRIT_MODE: 'golubin_hrit_bot',
    config.PR_MODE: 'golubin_pr_bot',
    config.VOCALS_MODE: 'golubin_vocals_bot',
    config.TAILORING_MODE: 'golubin_tailoring_bot',
    config.TRANSLATE_MODE: 'golubin_translate_bot',
    config.CUSTOMS_MODE: 'golubin_customs_bot',
    config.FURNITURE_MODE: 'golubin_furniture_bot',
    config.MUSIC_MODE: 'golubin_music_bot',
    config.TECH_FIXING_MODE: 'golubin_tech_fixing_bot',
    config.GEN_WROKERS_MODE: 'golubin_gen_workers_bot',
    config.HR_MODE: 'golubin_hr_bot',
    config.CLEANNING_MODE: 'golubin_cleanning_bot',
    config.BUSINESS_SALE_MODE: 'golubin_business_sale_bot',
    config.REELSMAKER_MODE: 'golubin_reelsmaker_bot'
}
MESSAGE_FILTER = 'bot_filter.json' if not config.TESTING else 'tests/bot_filter.json'
...
CHANNELS_OF_CATEGORIES = {
    config.TARGET_MODE: -1001917355681,
    config.SMM_MODE: -1001934664610,
    config.COPYRIGHT_MODE: -1001944818545
}
