...
from src.apis.db import accounts, promocodes, user
from src.data.bot_data import *
from src.data.config import TARGET_MODE
from src.keyboards.bot_keyboards import *

# @dp.message_handler(lambda message: message.text == PERSONAL_CABINET, state = '*')
async def get_personal_panel(message: Message, state: FSMContext):
    await state.finish()
    try:
        await message.answer(PERSONAL_CABINET_TEXT, reply_markup = personal_cabinet_kb, parse_mode = 'HTML')
    except BotBlocked:
        return
    
# @dp.message_handler(lambda message: message.text == ADDITIONAL_OPTIONS, state = '*')
async def get_additional_options_kb(message: Message, state: FSMContext):
    await state.finish()
    try:
        await message.answer(ADDITIONAL_OPTIONS_TEXT, reply_markup = additional_options_kb, parse_mode = 'HTML')
    except BotBlocked:
        return

# @dp.message_handler(lambda message: message.text == MAIN_MENU, state = '*')
async def get_main_menu(message: Message, state: FSMContext):
    await state.finish()
    try:
        await message.answer(MAIN_MENU_TEXT, reply_markup = main_kb, parse_mode = 'HTML')
    except BotBlocked:
        return
        
# @dp.message_handler(lambda message: message.text == NEW_ORDER, state = '*')
async def new_order(message: Message, state: FSMContext):
    await state.finish()
    kb = get_kb_with_categories()
    try:
        await message.reply(CHOOSE_PERIOD_TEXT, reply_markup = kb)
    except (BotBlocked, BadRequest):
        return
    else:
        user.set_date_of_trying_to_buy(message.from_user.id, message.date)
...
# @dp.callback_query_handler(lambda c: c.data.startswith(CHOOSE_CATEGORY), state = '*')
async def choose_order_category(callback: CallbackQuery, state: FSMContext):
    ...
    eternal_pcd = promocodes.get_last_activated_endless_promocode(callback.from_user.id, int(category))
    kb = InlineKeyboardMarkup()
    kb.row(
        InlineKeyboardButton('Да', callback_data=CALLBACK_SEP.join([USE_PROMOCODE_BUTTON, category, ''])),
        InlineKeyboardButton('Нет', 
                             callback_data=CALLBACK_SEP.join(
                                 [NEW_ORDER_WITHOUT_PROMOCODE, category] if not eternal_pcd else \
                                 [USE_PROMOCODE_BUTTON, category, eternal_pcd]
                             ))
    )
    ...
    try:
        await callback.bot.edit_message_text(
            text, callback.from_user.id, callback.message.message_id, reply_markup=kb, parse_mode='HTML')
    except:
        pass
    

# @dp.callback_query_handler(lambda c: c.data.startswith(NEW_ORDER_WITHOUT_PROMOCODE), state = '*')
async def send_new_order_wo_prmcd(callback: CallbackQuery, state: FSMContext):
    ...
    elif category == TARGET_MODE and user.date_of_recieveing_latest_messages(callback.from_user.id) == None:
        with_get_last_msgs_button = True
    ref_link = get_referal_link_of_client(callback.from_user.id)
    kb = get_period_choosing_keyboard(
        category, sales, with_sales_during_trial,
        from_client = callback.from_user.id, requested_from = CHOOSE_CATEGORY+CALLBACK_SEP+str(category),
        with_get_last_msgs_button = with_get_last_msgs_button, referal_link=ref_link
    )
    await callback.bot.edit_message_text(text,
...
async def choosing_period_handler(callback : CallbackQuery, state: FSMContext):
    ...
    if admin:
        await callback.answer('Вы не можете оплачивать подписки, '
    ...
    _, final_text, kb = generate_text_and_keboard_for_payment(mode, period, cost, sales, callback.from_user.id,
        with_sale_during_trial, requested_from, callback.message.message_id, referal_link)
...
async def select_category_to_trial(callback: CallbackQuery):
    await callback.message.answer(CHOOSE_CATEGORY_MESSAGE, reply_markup=get_kb_with_categories(ACTIVATE_TRIAL, requested_from=NEW_ORDER_BUTTON))
...
async def make_trial(callback: CallbackQuery, state: FSMContext):
    ...
    save_to_statistics(
        new_users = int(not cl_is_in_any_db), activated_trial = 1, ref_link = parse_referal_link(referal_link))
    await callback.bot.send_message(client_id, 
        f'Срок пробного периода истекает {client.payment_end.strftime(PAYMENT_END_DATE_FORMAT)}.\n\n'
        f'{TEXT_AFTER_FIRST_PAYMENT.format(callback.bot_names[client.sending_mode])}\n\nДля того, '
    ...
    if not date_of_recieveing_latest_messages or (datetime.now() - date_of_recieveing_latest_messages).days >= 2:
        await try_to_send_latest_messages(client)

def client_is_in_db(client_id: int, mode: int) -> typing.Tuple[bool, Union[Client, None]]: 
    ''' 
    returns: 
...
def get_channel_for_trial_period() -> str:
    with open(callback.bot_DATA_FILE) as f:
...
def parse_referal_link(referal_link: str) -> str:
    if referal_link.startswith(REFERAL_ID):
        return f't.me/{PAYING_callback.bot_NICK}?start=ref{referal_link[1:]}'
...
async def send_article_if_it_is_promised(client_id: int):
    ...
    if user.user_is_offered_trial(client_id): 
        await callback.bot.send_message(client_id, 
            ...
            'Переходи и читай - будет полезно!', parse_mode = 'HTML'
        )
...
async def back_to_period_choosing(callback: CallbackQuery, state: FSMContext):
    ...
    requested_from = CALLBACK_SEP.join(args[5:]) # in case requested_from is callback string (contains separators)
    mode = int(mode)
    ...
    if sales:
        ...
        for p in days_per_period:
            ...
            if sale:
                text += p + ' дн. - ' + str(delete_float_point_if_is_not_fraction(sale*100)) + '%\n'
    ...
    try:
        await callback.bot.edit_message_text(
            ...
            reply_markup = get_period_choosing_keyboard(
                mode, sales, with_sales_during_trial, requested_from = requested_from,
                from_client = callback.from_user.id, referal_link = referal_link
            ), parse_mode = 'HTML'
        )
    except:
        pass
...
async def pay(callback : CallbackQuery, state: FSMContext):
    await state.finish()
    args = callback.data.split(CALLBACK_SEP)
    _, str_period, label, mode, with_sale_during_trial, referal_link, required_transfers_num = args
    mode = int(mode)
    with_sale_during_trial = int(with_sale_during_trial)
    ...
    if len(history.operations):
        ...
        if operations_num < int(required_transfers_num):
            ...
            return
        cost = sum([Decimal(str(operation.amount)) for operation in history.operations])
    elif TESTING:
    ...
    if TEXT_ABOUT_REFERAL_WITHDRAWAL_FOR_PAYMENT in callback.message.text:
        referal_fee = Decimal(callback.message.text.split('\n')[-1].split()[0][1:].replace(',', '.'))
    else:
        referal_fee = Decimal(0)
    try:
        await process_payment(callback, cost, period, mode, with_sale_during_trial, label, referal_fee, referal_link)
...
async def process_payment(
    ...
    if try_to_edit:
        try:
            ...
            logging.exception(f'Failed to delete message ({chat_id}, {msg_id}) when process {period} days')
            return
    commission = 0
    try:
        commission = await process_referal(client_id, cost, callback.bot)
    except Exception as e:
        logging.error(f'Process referal for {client_id} is failed: '+str(e), exc_info = True)
    clients = Client.get_client_by_id(id = client_id)
    ...
    try:
        payment = Payment(period, cost, commission, callback.from_user.id, category = category, 
    ...
    await notify_about_new_payment(callback, category, period, cost, nick, referal_fee)
    now = datetime.now()
    last_payment_end = datetime(1000, 1, 1, 1, 1)
    bonus_days = BONUS_DAYS_AT_THE_END_OF_TRIAL if with_sale_during_trial and \
        str(period) == PERIOD_WITH_BONUS_DAYS_AFTER_THE_END_OF_TRIAL else 0
    try:
        if current_client:
            ...
            if current_client.payment_end.date() >= datetime.now().date():
                ...
                current_client.payment_end = now + timedelta(days = period + bonus_days)
            current_client.last_payment_date = now
        else:
            current_client = Client(client_id, now, 
                now + timedelta(days = period + bonus_days), category)
        had_paid_period_for_current_category = current_client.has_paid_period
        current_client.add_to_db()
        ...
        await callback.bot.send_message(DEVELOPER_ID, 
            error_message + \
    ...
    text = \
        ...
        f'{TEXT_AFTER_FIRST_PAYMENT.format(callback.bot_names[current_client.sending_mode])}'
    if referal_fee:
        text += f'\n{delete_float_point_if_is_not_fraction(referal_fee)} руб. списано с вашего реферального счёта'
        ReferalPaymentsHistory.save(ReferalPayment(date.today(), callback.from_user.id, cost))
        save_to_statistics(
            total_referal_commisions = referal_fee, total_referal_income = referal_fee, 
            new_referal_buyers = not ReferalPaymentsHistory.clientHasPayments(callback.from_user.id), 
    ...
    if not had_paid_period_for_current_category:
        try:
            surcharge_kb, surcharge_amount, new_period = \
                get_kb_to_surcharge_extra_days(str(period), category, payment.id, referal_link)
            if surcharge_kb:
                difference = get_period_cost_without_sales(new_period, category) - \
                ...
                await callback.bot.send_message(
                    callback.from_user.id, text, reply_markup = surcharge_kb, parse_mode = 'HTML')
        except  Exception as e:
            logging.error('Failed to offer client surcharging: '+str(e), exc_info = True)
    else:
        await callback.bot.send_message(current_client.id, 'Главное меню', reply_markup = main_kb)
    save_to_statistics(
    ...
    date_of_recieveing_latest_messages = user.date_of_recieveing_latest_messages(current_client.id)
...
async def notify_about_new_payment(callback: CallbackQuery, mode: int, period: 
    int, cost: Decimal, nick: str, referal_fee: Decimal
):
    await callback.bot.send_message(
        ...
        (f' за {delete_float_point_if_is_not_fraction(cost)} руб.' if cost else '') + \
        ('' if not referal_fee else f' (<b>{delete_float_point_if_is_not_fraction(referal_fee)}</b> руб'
            'оплачено с реферального счёта)'),
            parse_mode = 'HTML'
    )

def get_kb_to_surcharge_extra_days(current_period: str, mode: int, payment_id: int, ref_link: str) -> tuple:
    ...
    surcharge_amount = get_period_cost(new_period, mode) - get_period_cost(current_period, mode)
    ...
    kb.add(
        InlineKeyboardButton(
            'Увеличить период', 
            callback_data = CALLBACK_SEP.join(
                [SURCHARGE, str(surcharge_amount), str(days_to_expand), str(current_period), 
                    str(mode), str(payment_id), ref_link]
            )
        )
    )
...
async def pay_using_referal_balance(callback: CallbackQuery):
    if not RefClient.has_client_id(callback.from_user.id):
        ...
        return
    args = callback.data.split(CALLBACK_SEP)
    _, period, category, cost, with_sale_during_trial, sales, referal_link = args[:7]
...
# @dp.message_handler(state = States.get_sum_to_withdraw_from_referal_balance_for_payment)
async def withdraw_ref_balance(message: Message, state: FSMContext):
    try:
        sum = Decimal(message.text.replace(',', '.'))
    except:
        await message.answer('Введено некоректное значение, попробуйте снова, или отправьте /cancel')
        return
    ...
    ref_client = RefClient.get_client_by_id(message.from_user.id)
    if not ref_client:
        await message.answer('У вас нет реферального счёта')
        await state.finish()
        return
    ...
    if sum == data['cost']:
        ...
        try:
            await process_payment(
                data['callback'], data['cost'], int(data['period']), data['category'], False, referal_fee=sum)
        except:
            period = data['period']
            cost = data['cost']
            logging.critical(
                f'Failed to save client`s ({ref_client.id}) {period} d. period which costs {cost} using '
                'referal balance:', exc_info=True
            )
    else:
        ref_client.balance -= sum
        ref_client.add_to_db()
        try:
            is_allowed_to_pay, text, kb = generate_text_and_keboard_for_payment(
                data['category'], data['period'], data['cost'] - sum, data['sales'], message.from_user.id,
                data['with_sale_during_trial'], data['requested_from'], data['callback'].message.message_id,
                data['referal_link'], False
            )
        except:
            logging.critical(f'Failed to generate keyboard and text for referal-paying:', exc_info=True)
        try:
            await data['callback'].message.delete()
        except:
            try:
                await data['callback'].message.edit_text(
                    'Оплатите через клавиатуру снизу' if is_allowed_to_pay else '...')
...
# @dp.callback_query_handler(lambda c: c.data.startswith(USE_PROMOCODE_BUTTON), state = '*')
async def get_promocode(callback: CallbackQuery, state: FSMContext):
    ...
    if not promocode:
        text = f'Вы выбрали категорию {message_category_names[int(category)]}. '\
        ...
        kb.add(InlineKeyboardButton(
            BACK_BUTTON_TEXT, callback_data = CALLBACK_SEP.join([CHOOSE_CATEGORY, category])))
        await callback.bot.edit_message_text(
            text,
        ...
        await check_given_promocode(promocode, int(category), callback.from_user)

# @dp.message_handler(state = States.get_promocode)
async def get_sent_promocode(message: Message, state: FSMContext):
    promocode = message.text.lower()
    chosen_category = (await state.get_data())['chosen_category']
    await state.finish()
    await check_given_promocode(promocode, chosen_category, message.from_user)

async def check_given_promocode(promocode: str, chosen_category: int, from_user: User):
    if not promocode_is_in_db(promocode):
        await callback.bot.send_message(from_user.id, f'Промокод "{promocode}" не существует')
        return
    ...
    if not pcd_is_endless and pcd_has_been_used:
        await callback.bot.send_message(from_user.id, f'Вы уже использовали промокод "{promocode}"')
        return
    category, trial_extra_days, sale, period, due_time = get_promocode_action(promocode)
    ...
    if not pcd_is_endless and due_time and datetime.now() >= due_time:
        await callback.bot.send_message(from_user.id, 'Время действия промокода истекло')
        return
    if not pcd_has_been_used: # also if it is not endless
        try:
            set_promocode_as_used_by_client(from_user.id, promocode)
        except:
            logging.critical(f'Failed to set promocode "{promocode}" as used by {from_user.id}: ', exc_info = True)
    promocode_id = get_id_of_promocode(promocode)
    try:
        if trial_extra_days:
            await process_using_trial_promocode(promocode, category, trial_extra_days, from_user.id)
        elif sale:
            await process_payment_sale_promocode(promocode_id, category, sale, str(period), from_user.id)
    except:
        logging.critical(
            f'Failed to process promocode "{promocode}" for {from_user.id}: ', exc_info = True)
    try:
        process_referal_for_promocode(promocode, from_user.id)
    except Exception as e:
        logging.error(f'Failed to check refer of promocode "{promocode}" for {from_user.id}')

async def process_using_trial_promocode(promocode: str, category: Union[int, None], trial_extra_days: int, client_id: int):
    client = Client.get_clients_by_filter(id = client_id, category = category)
    now = datetime.now()
    if not client:
        client = Client(client_id, now, now + timedelta(days = trial_extra_days), category)
        client.add_to_db()
    else:
        await callback.bot.send_message(client_id, 'Вам больше не доступен пробный период')
        return
    ...
    await callback.bot.send_message(
        ...
        f'<b>{message_category_names[category]}</b>', parse_mode = 'HTML'
    )
    if not Client.did_activate_callback.bot(client):
        await callback.bot.send_message(client.id, f'Не забудьте активировать бота @{callback.bot_names[client.sending_mode]}')
    statistics.save_to_statistics(new_users = 1, activated_trial = 1, trial_activations = 1, ref_link=promocode)
    user.set_user_as_offered_trial(client_id)
    date_of_recieveing_latest_messages = user.date_of_recieveing_latest_messages(client.id)
    if not date_of_recieveing_latest_messages or (datetime.now() - date_of_recieveing_latest_messages).days >= 2:
        await try_to_send_latest_messages(client)
...
async def process_payment_sale_promocode(
        promocode_id: int, category: int, sale: Union[Decimal,float], period: str, client_id: int):
    sales = {period: sale}
    tariff = f'на тариф <b>{period}</b> дн.\n' if period != '*' else 'на все тарифы\n'
    await callback.bot.send_message(
        client_id,
        ...
        tariff + \
        '(Цены указаны с учётом скидки)',
        reply_markup = get_period_choosing_keyboard(category, sales = sales, 
            from_client = client_id, referal_link = f'{PROMOCODE_ID}{promocode_id}'),
        parse_mode = 'HTML'
    )
...
# @dp.message_handler(lambda message: message.text == SUBSRIBES_BUTTON, state = '*')
async def get_subscribes(message: Message, state: FSMContext):
    await state.finish()
    text, keyboard = await generate_message_for_subscribes_tab(message.from_user)
    await message.reply(text, reply_markup = keyboard if keyboard.inline_keyboard else None, parse_mode = 'HTML')
...
async def generate_message_for_subscribes_tab(user: User):
    ...
    for sub in client:
        ...
        if now.year == sub.last_payment_date.year == sub.payment_end.year:
            text += f'с <b>{sub.last_payment_date.strftime("%d.%m %H:%M")}</b> '\
                f'по <b>{sub.payment_end.strftime("%d.%m %H:%M")}</b> '
        else:
            text += f'с <b>{sub.last_payment_date.strftime("%d.%m.%Y %H:%M")}</b> '\
    ...
    if len(categories) > 1:
        ...
        kb.add(InlineKeyboardButton(
            '⏳ Приостановить подписку', callback_data = CALLBACK_SEP.join(
                [PAUSE_PERIOD, str(categories[0]), SUBSCRIBES_CALLBACK])
            )
        )
    if has_paid_periods:
        kb.add(InlineKeyboardButton('Перенести клиент на другой акк.', callback_data = TRANSFER_ACCOUNT))
    return text, kb
...
async def get_category_to_pause_subscribe(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    now = datetime.now()
    kb = InlineKeyboardMarkup()
    ...
    for sub in subscriptions:
        ...
        kb.add(
            InlineKeyboardButton(
                message_category_names[int(sub.sending_mode)], callback_data = \
                    CALLBACK_SEP.join([PAUSE_PERIOD, str(sub.sending_mode), callback.data])
            )
        )
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = SUBSCRIBES_CALLBACK))
    await callback.message.edit_text(text_about_subscribe_pauses + '\n\nВыберите подписку: ', reply_markup = kb)
...
async def get_days_to_pause_period(callback: CallbackQuery, state: FSMContext):
    ...
    client = Client.get_clients_by_filter(id = callback.from_user.id, category = category, is_using_trial = False)
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = back_button_callback))
...
async def pause_period(message: Message, state: FSMContext):
    if not message.text.isdigit() or '-' in message.text:
        await callback.bot.send_message(message.from_user.id, 'Не корректное значение, попробуйте ещё раз')
        return
    data = await state.get_data()
    category = data['category']
    await state.finish()
    client = Client.get_clients_by_filter(id = message.from_user.id, category = category)[0]
    ...
    if client.used_pause_days + day_number > client.max_pause_days:
        ...
        await state.update_data(category = category)
        await callback.bot.send_message(
            client.id, f'Введёное значение слишком большое. Отправьте другое значение, или /cancel')
        return
    try:
        ...
        logging.critical(
            f'Failed to increase pause day for {client.id} by {day_number} d.: '+str(e), exc_info = True)
    else:
        await callback.bot.send_message(
            client.id,
...
# @dp.callback_query_handler(lambda callback: callback.data == TRANSFER_ACCOUNT, state = '*')
async def get_tg_id_to_transfer(callback: CallbackQuery, state: FSMContext):
    ...
    if admin:
        try:
            ...
            nick = admin
        await callback.message.edit_text(f'Вы не можете совершить трансфер, так как ваш аккаунт подключен к {nick}',
            parse_mode = 'HTML', reply_markup = kb)
        await state.finish()
        return
    await callback.message.edit_text('Отправьте ID аккаунта, на который хотите выполнить трансфер.\n\n'
        '<u>ID аккаунта - это не ник</u>. Для получения ID-аккаунта нужно запустить @getmyid_callback.bot, бот выдаст вам сообщение \n\n'
...
async def get_confirmation_to_transfer(message: Message, state: FSMContext):
    try:
    ...
    if new_id == message.from_user.id:
        await message.answer('Вы ввели свой же ID')
        return
    await state.finish()
    try:
    ...
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton('Подтвердить', callback_data = f'{SEND_TRANSFER_CONFIRMATION};{new_id}')
    )
    kb.add(
        InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = TRANSFER_ACCOUNT)
    )
    await message.answer(f'Вы хотите перенести все данные на {nick}?' + \
        ('' if isinstance(nick, str) else ' (не удаётся получить имя аккаунта. Возможно аккаунта с таким '
            'ID не существует, либо на нём не запущен этот бот)'), parse_mode = 'HTML', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(SEND_TRANSFER_CONFIRMATION), state='*')
async def send_validation_to_transfer(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    new_id = int(callback.data.split(';')[1])
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton('Подтвердить', callback_data = f'{CONFIRM_TRANSFER_SUB};{callback.from_user.id}')
    )
    nick = await get_nick(callback.from_user)
    try:
        msg = await callback.bot.send_message(
            new_id, f'{nick} хочет перенести свой клиент на ваш аккаунт.\n'
        ...
        await callback.message.edit_text('Не удалось отправить подтверждение на этот аккаунт', 
            reply_markup = kb)
        return
    await callback.message.edit_text(f'Подтверждение о переносе данных отправленно на {msg.chat.full_name}. '
        'Откройте его и нажмите "Подтвердить", чтобы завершить трансфер.')

# @dp.callback_query_handler(lambda callback: callback.data.startswith(CONFIRM_TRANSFER_SUB), state='*')
async def transfer_account(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        nick = new_id
    await callback.message.edit_text(f'Все данные с {nick} полностью перенесенны',
        parse_mode='HTML', reply_markup = kb)
    await callback.bot.send_message(old_id, 
        f'Все данные перенесены на {await get_nick(callback.from_user)}', parse_mode = 'HTML')
    ...
    try:
        Votes.Vote.transferVotes(old_id, new_id)
    except:
...
# @dp.message_handler(lambda message: message.text.endswith(TAB_HELPFUL_BUTTON_FOR_NEWBIE), 
    state = '*')
async def get_helpful_tab(message: Message, state:  FSMContext):
    await state.finish()
    await message.reply(
        'Полезное:', 
...
def get_kb_for_helpful_tab():
    ...
    kb.add(InlineKeyboardButton('Статья с обзором бота', url= 'https://vk.com/@golubin_callback.bot-obzor-servisa-golubin-callback.bot'))
    kb.add(InlineKeyboardButton('Информация о боте', callback_data = callback.bot_INFO))
    return kb

# @dp.callback_query_handler(lambda call: call.data == TAB_HELPFUL_CALLBACK, state = '*')
...
# @dp.callback_query_handler(lambda call: call.data.startswith(callback.bot_INFO), state = '*')
async def get_callback.bot_info(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    if callback.message.text == TEXT_ABOUT_callback.bot:
        return
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton('Оформить подписку', callback_data = NEW_ORDER_BUTTON))
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = TAB_HELPFUL_CALLBACK))
    await callback.bot.edit_message_text(
        TEXT_ABOUT_callback.bot,
        callback.from_user.id, callback.message.message_id, reply_markup = kb
    )

# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_INFO_ABOUT_callback.bot_FOR_NEWBIE), state = '*')
async def get_info_about_callback.bot(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    if callback.message.text == TEXT_ABOUT_callback.bot:
        return
...
# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_LAST_MSGS_PER_DAY), state = '*')
async def get_category_to_send_last_messages(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    admin = accounts.get_admin_of_account(callback.from_user.id)
    if admin:
    ...
    try:
        ...
        logging.critical(f'Error occured while sending latest  messages to user({user_id}):', exc_info = True)
    try:
        await callback.message.delete()
    except:
        pass
...
# @dp.callback_query_handler(lambda callback: callback.data.startswith(KEYBOARD_WITH_ANSWERS), state = '*')
async def set_keybaord_with_answers_why_didnt_by(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    category = int(callback.data.split(CALLBACK_SEP)[1])
    ...
    if callback.message.text != text:
        await callback.message.edit_text(
            text, 
            reply_markup = kb_with_answers_why_didnt_buy_period(category, InlineKeyboardMarkup, InlineKeyboardButton)
        )

# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_ANSWER_WHY_DIDNT_BUY), state = '*')
async def get_answer_why_didnt_buy(callback: CallbackQuery, state: FSMContext):
    ...
    if i == ANSWER_WITH_ENROLL_IN_COURSE:
        kb.add(InlineKeyboardButton('Записаться', callback_data = ENROLL_IN_COURSE+CALLBACK_SEP+category))
    elif i == ANSWER_WITH_GET_REPORT:
        kb.add(InlineKeyboardButton('Отправить отчёт', callback_data = REPORT_ERROR_BUTTON+CALLBACK_SEP+'1'))
    elif i == ANSWER_WITH_GET_MESSAGE_TO_DEVELOPER:
        kb.add(InlineKeyboardButton('Отправить отчёт', callback_data = REPORT_ERROR_BUTTON+CALLBACK_SEP+'2'))
    else:
        kb.add(InlineKeyboardButton(
            NEW_ORDER, callback_data = NEW_ORDER_BUTTON+CALLBACK_SEP+GET_ANSWER_WHY_DIDNT_BUY))
    answer = list(ANSWERS_WHY_DIDNT_BUY.keys())[i]
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = KEYBOARD_WITH_ANSWERS+CALLBACK_SEP+category))
    try:
        await callback.message.edit_text(ANSWERS_WHY_DIDNT_BUY[answer], reply_markup = kb)
    except (MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty, MessageToEditNotFound):
        pass

# @dp.callback_query_handler(lambda callback: callback.data.startswith(ENROLL_IN_COURSE), state = '*')
async def enroll_client_in_course(callback: CallbackQuery, state: FSMContext):
    ...
    kb.add(
        InlineKeyboardButton(
            BACK_BUTTON_TEXT, 
            callback_data = CALLBACK_SEP.join(
                [GET_ANSWER_WHY_DIDNT_BUY, str(ANSWER_WITH_ENROLL_IN_COURSE), category])
        )
    )
    await States.get_date_of_course.set()
    await callback.message.edit_text(
        'Выберите желаемую дату консультации (/cancel для отмены):',
        reply_markup = kb
    )

# @dp.callback_query_handler(simple_cal_callback.filter(), state = States.get_date_of_course)
async def get_date_of_course(callback: CallbackQuery, callback_data: dict):
    ...
    kb = InlineKeyboardMarkup()
    kb.row(
        InlineKeyboardButton('Утро', callback_data = CHOOSE_CONSULTATION_TIME+CALLBACK_SEP+MORNING),
        InlineKeyboardButton('Обед', callback_data = CHOOSE_CONSULTATION_TIME+CALLBACK_SEP+HIGH_NOON),
        InlineKeyboardButton('Вечер', callback_data = CHOOSE_CONSULTATION_TIME+CALLBACK_SEP+EVENING)
    )
    kb.add(
        InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = ENROLL_IN_COURSE)
    )
    await callback.message.edit_text(
...
# @dp.callback_query_handler(
    lambda callback: callback.data.startswith(CHOOSE_CONSULTATION_TIME+CALLBACK_SEP), 
...
async def choose_time(callback: CallbackQuery, state: FSMContext):
    ...
    label = blake2b((str(datetime.now())+' '+time+str(callback.from_user.id)).encode('utf-8'), digest_size = 10).hexdigest()
    str_date = f'{time} {date.strftime("%d.%m.%Y")}'
    quickpay = Quickpay(
        receiver=ym_user.account,
        quickpay_form="shop",
        targets=\
            f"Запись на консультацию ({str_date}, {consultation_time[time]})",
        paymentType = "SB",
        sum = COURSE_PRICE,
        label = label
    )
    ...
    kb.row(
        ...
        InlineKeyboardButton(
            'Записаться ✅', callback_data = CALLBACK_SEP.join([NOTFIY_ABOUT_NEW_COURSE, label, str_date]))
    )
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = ENROLL_IN_COURSE))
    await callback.message.edit_text(
        f'Вы выбрали консультацию на {str_date}, {consultation_time[time]}', reply_markup = kb)

# @dp.callback_query_handler(
    lambda callback: callback.data.startswith(NOTFIY_ABOUT_NEW_COURSE+CALLBACK_SEP), state = '*')
async def notify_about_new_consultation(callback: CallbackQuery, state: FSMContext):
    ...
    if not len(history.operations) and not TESTING:
        await callback.bot.send_message(
            callback.from_user.id, 'Оплата не была произведена, либо произошла ошибка')
        return
    try:
        await callback.bot.send_message(
            EUGENIY_ID if not TESTING else callback.from_user.id, 
        ...
        logging.critical(
            f'Failed to notify about new consulation payment from {callback.from_user.id}: '+str(e), 
            exc_info = True
        )
    try:
        await callback.message.edit_text(f'Спасибо за оплату, в ближайшее время я свяжусь'
            ' с тобой в личных сообщениях и мы договоримся о созвоне')
    except Exception as e:
        logging.critical(f'Failed to answer new consultation payment: '+str(e), exc_info = True)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(SURCHARGE), state = '*')
async def get_surcharge(callback: CallbackQuery, state: FSMContext):
    ...
    if (datetime.now() - callback.message.date).seconds >= SURCHARGE_PERIOD:
        try:
            ...
            pass
        return
    _, surcharge_amount, days_to_expand, old_period, mode, payment_id, referal_link = callback.data.split(CALLBACK_SEP)
    new_period = str(int(days_to_expand) + int(old_period))
    mode = int(mode)
    kb = InlineKeyboardMarkup()
    ym_user = ym_client.account_info()
    label = blake2b(
        (str(callback.from_user.id)+surcharge_amount+days_to_expand+str(datetime.now())+new_period
    ).encode('utf-8'), digest_size = 10).hexdigest()
    quickpay = Quickpay(
        receiver=ym_user.account,
        quickpay_form="shop",
        targets=\
            f"Доплата подписки на Golubin бота до {new_period} дн. по категории {message_category_names[mode]}",
        paymentType = "SB",
    ...
    kb.row(
        InlineKeyboardButton('Оплатить', url = quickpay.redirected_url),
        InlineKeyboardButton(
            'Готово ✅', callback_data = CALLBACK_SEP.join(
                                                                                    #surcharge amount
                [EXPAND_PERIOD,days_to_expand,str(old_period), label, str(mode), '', '0', payment_id,referal_link]) 
        )                                                                             #is paing_using_referal balance
    )
    kb.add(
        InlineKeyboardButton(
            'Оплатить с реферального счёта', callback_data = CALLBACK_SEP.join(
                [EXPAND_PERIOD, days_to_expand, str(old_period), '', str(mode), str(surcharge_amount), '1', 
                    payment_id, referal_link]
            )
        )
    )
    try:
...
# @dp.callback_query_handler(lambda callback: callback.data.startswith(EXPAND_PERIOD+CALLBACK_SEP), state = '*')
async def expand_period(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    _,days_to_expand,old_period,label,category,surcharge_amount,is_paying_using_ref_balance,payment_id,ref_link=\
    ...
    category = int(category)
    client = await try_to_find_client_to_expand_period(callback.from_user, category, days_to_expand)
    if not client:
        try:
            await callback.message.edit_text(
            ...
            pass
        return
    if not await try_to_expand_client_period(client, days_to_expand, callback.from_user):
        return
    if is_paying_using_ref_balance:
        ref_client.balance -= surcharge_amount
        ref_client.add_to_db()
        ReferalPaymentsHistory.save(ReferalPayment(date.today(), callback.from_user.id, surcharge_amount))
    ...
    try:
        await callback.message.edit_text(
            ...
            ('' if not is_paying_using_ref_balance else \
    ...
    commission = 0
    try:
        commission = await process_referal(callback.from_user.id, surcharge_amount, callback.bot)
    except:
        logging.error(f'Process referal for {callback.from_user.id} is failed: ', exc_info = True)
    try:
        save_payment_after_expanding_period(
            int(old_period), days_to_expand, client, surcharge_amount, payment_id, commission, ref_link)
    except:
        logging.error(f'Failed to save payment:', exc_info=True)
    

async def try_to_find_client_to_expand_period(user: User, mode: int, days_to_expand: int) -> Client:
    try:
        client = Client.get_clients_by_filter(id = user.id, category = mode)[0]
    except Exception as e:
        logging.critical(
            f'Failed to find client({user.id}) after surcharging for {days_to_expand} d.: '+str(e),
            exc_info = True
        )
        await callback.bot.send_message(DEVELOPER_ID, f'Failed to find {await get_nick(user)} after surcharging')
    return client

async def try_to_expand_client_period(client: Client, days_to_expand: int, user: User) -> bool:
    try:
        client.increase_period(days_to_expand)
    except Exception as e:
        logging.critical(
            f'Failed to expand client`s ({user.id}) period by {days_to_expand} days: '+str(e),
            exc_info = True
        )
        await callback.bot.send_message(
            DEVELOPER_ID,
...
def save_payment_after_expanding_period(old_period: int, days_to_expand: int, 
    client: Client, surcharge_amount: Decimal, payment_id: int, referal_commision: Decimal, referal_link: str
):
    payment_amount = 0
    comments = IS_UPSALED
    payment = None
    try:
        ...
        logging.critical('EXPAND PAYMENT ERROR: failed to find payment while expanding:', exc_info = True)
    try:
        if not payment:
            logging.warning('EXPAND PAYMENT ERROR: Can not find payment after expanding period '
                f'(from {old_period} to {int(old_period) + days_to_expand}) for {client.id}')
            payment_amount = {int(v): k for k,v in list(surcharges.items())}[int(surcharge_amount)]
        else:
            ...
            try:
                ...
                logging.critical('EXPAND PAYMENT ERROR: failed to delete payment while expanding:', exc_info = True)
        PaymentHistory.savePayment(
            Payment(client.payment_period, payment_amount+surcharge_amount, payment[0].referal_commission + referal_commision, 
                client.id, comments = comments, category = client.sending_mode, referal_link=payment[0].referal_link)
        )
    except:
        logging.critical(f'Failed to save payment after expanding period {client.id, surcharge_amount, old_period + days_to_expand}:',
            exc_info=True)
    save_to_statistics(
...
# @dp.message_handler(lambda message: message.text == STOP_WORDS, state = '*')
async def stop_words_message_handler(message: Message, state: FSMContext):
    await state.finish()
    admin = accounts.get_admin_of_account(message.from_user.id)
    if admin:
        await message.answer('Вы не можете использовать стоп слова, '
    ...
    await message.reply('Выберите направление:', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data == STOP_WORDS_CALLBACK, state = '*')
...
# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_CATEGORY_FOR_FILTER), state = '*')
async def stop_words(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    category = callback.data.split(CALLBACK_SEP)[1]
    stop_words_about_text = f'Вы перешли в меню настройки стоп-слов для категории ' +\
        f'{message_category_names[int(category)]}. Выберите действие:'
    if callback.message.text == stop_words_about_text:
        return
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton('Текущий фильтр', callback_data = SHOW_STOP_WORDS+CALLBACK_SEP+category)
    )
    kb.row(
        InlineKeyboardButton('✅ Добавить', callback_data = ADD_STOP_WORDS+CALLBACK_SEP+category),
        InlineKeyboardButton('⛔️ Удалить', callback_data = DELETE_STOP_WORDS+CALLBACK_SEP+category)
    )
    kb.add(
        InlineKeyboardButton('❌ Очистить фильтр', callback_data = CLEAR_STOP_WORDS+CALLBACK_SEP+category)
    )
    kb.add(
        InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = STOP_WORDS_CALLBACK)
    )
    await callback.message.edit_text(
        stop_words_about_text, reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(SHOW_STOP_WORDS), state = '*')
async def show_all_stop_words(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup()
    if not stop_words:
        kb.add(InlineKeyboardButton('Создать', callback_data = ADD_STOP_WORDS+CALLBACK_SEP+str(category)))
    kb.add(InlineKeyboardButton(
        BACK_BUTTON_TEXT, callback_data = GET_CATEGORY_FOR_FILTER+CALLBACK_SEP+str(category)))
    if len(stop_words) > 50:
        await callback.bot.send_document(client.id, InputFile(StopWords.get_path_to_filter(client), 'фильтр.txt'))
        await callback.message.edit_text(f'{len(stop_words)} слов всего:', reply_markup = kb)
        return
    if stop_words and stop_words != [''] and stop_words != [' ']:
        text = ''
        for w in stop_words:
            text += '\t'+w+'\n'
        await callback.message.edit_text('Ваш текущий список стоп-слов: \n'+text, reply_markup = kb)
    else:
        await callback.message.edit_text('У вас нет фильтра. Вы можете создать его:', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(ADD_STOP_WORDS), state = '*')
async def add_stop_words(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(category = category)
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = GET_CATEGORY_FOR_FILTER+CALLBACK_SEP+str(category)))
    await callback.message.edit_text(
        'Отправьте слово.\n'
...
async def get_new_stop_word(message: Message, state: FSMContext):
    category = (await state.get_data())['category']
    await state.finish()
    client = Client(message.from_user.id, None, None, category)
    words = get_words_from_text(message.text)
    StopWords.add_stop_words(client, words)
    len_words = len(words)
    await callback.bot.send_message(
        client.id, 
        'Слов'+('o' if len_words == 1 else 'а')+' добавлен'+('o' if len_words == 1 else 'ы')+' в ваш фильтр '
...
# @dp.callback_query_handler(lambda callback: callback.data.startswith(DELETE_STOP_WORDS), state = '*')
...
async def get_stop_words_to_delete(message: Message, state: FSMContext):
    category = (await state.get_data())['category']
    await state.finish()
    ...
    if len(words) > 1:
        ...
        text = f'Слово удалено из вашего списка категории {message_category_names[category]}' \
            if not len(not_deleted) else 'Слово не найдено в вашем списке'
    await callback.bot.send_message(client.id, text)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(CLEAR_STOP_WORDS), state = '*')
async def clear_stop_words(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    category = int(callback.data.split(CALLBACK_SEP)[1])
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = GET_CATEGORY_FOR_FILTER+CALLBACK_SEP+str(category))
    )
    try:
        StopWords.clear_filter(Client(callback.from_user.id, None, None, category))
    except ValueError:
        await callback.message.edit_text(f'Ваш фильтр категории {message_category_names[category]} итак пуст.',
            reply_markup = kb)
    else:
        await callback.message.edit_text('Ваш фильтр категории {message_category_names[category]} очищен.', 
            reply_markup = kb)
            
# @dp.callback_query_handler(lambda callback: callback.data.startswith(WROTE_REVIEW), state = '*')
async def notify_about_new_review(callback: CallbackQuery, state: FSMContext):
    ...
    kb.row(
        InlineKeyboardButton(
            '1', 
            callback_data = CALLBACK_SEP.join(
                [SEND_BONUS_DAYS_FOR_REVIEW, str(callback.from_user.id), category, '1']
            )
        ),
        InlineKeyboardButton(
            '3', 
            callback_data = CALLBACK_SEP.join(
                [SEND_BONUS_DAYS_FOR_REVIEW, str(callback.from_user.id), category, '3']
            )
        ),
        InlineKeyboardButton(
            '7', 
            callback_data = CALLBACK_SEP.join(
                [SEND_BONUS_DAYS_FOR_REVIEW, str(callback.from_user.id), category, '7']
            )
        )
    )
    kb.add(
    ...
    try:
        ...
        logging.error(f'Failed to notify about new review from {callback.from_user.id}: '+str(e), exc_info = True)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(SEND_BONUS_DAYS_FOR_REVIEW), state = '*')
async def send_sale_for_review(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.bot.send_message(user_id, f'Ваш период по категории {message_category_names[category]} за отзыв расширен'
        f' на {bonus_days} дней (До {str_payment_end}).', parse_mode = 'HTML')
    await callback.message.edit_text(f'Пробные {bonus_days} дн. отправлены')

# @dp.callback_query_handler(lambda callback: callback.data.startswith(MARK_REVIEW_AS_FAKE), state = '*')
async def send_message_to_liar(callback: CallbackQuery, state: FSMContext):
    ...
    kb.add(
        InlineKeyboardButton(
            'Написал', 
            callback_data = CALLBACK_SEP.join([WROTE_REVIEW, category])
        )
    )
    await callback.bot.send_message(
        user_id, 'Мы не смогли найти ваш отзыв. Пожалуйста, оставьте свой ник в комментарии, '
        'чтобы мы смогли определить ваш аккаунт.\n\n'+URL_WITH_REVIEWS, reply_markup = kb
    )
    await callback.message.edit_text('Ссылка на отзыв повторно отправлена пользователю')

# @dp.message_handler(lambda message: message.text == POST_APPLICATION, state = '*')
async def get_category_of_application(message: Message, state: FSMContext):
    await state.finish()
    admin = accounts.get_admin_of_account(message.from_user.id)
    if admin:
        await message.answer('Вы не можете выкладывать заявки, '
            f'так как ваш аккаунт подключен к {await get_nick((await dp.callback.bot.get_chat_member(admin, admin)).user)}')
        return
    str_user_id = str(message.from_user.id)
    client_has_already_sent_app_to_check = await check_if_client_sended_application_sooner(
        str_user_id, 
        lambda keyboard: message.reply(
    ...
    if not client_has_already_sent_app_to_check:
        kb = get_kb_with_categories(for_callback = GET_CATEGORY_OF_POST, 
            text_for_back_button = POST_APPLICATION_CALLBACK)
        await message.reply('Выберите категорию вашей заявки:', reply_markup = kb)
    ...
    for category in listdir(APPLICATIONS_FROM_USERS_DIR):
        for file in listdir(APPLICATIONS_FROM_USERS_DIR+'/'+category):
            if file.startswith(str_user_id):
                ...
                kb = InlineKeyboardMarkup()
                kb.add(
                    InlineKeyboardButton('отменить проверку', 
                        callback_data = CALLBACK_SEP.join([DENY_CHECK_APPLICATION, file, str(msgc[category])]))
                )
                try:
...
# @dp.callback_query_handler(lambda callback: callback.data == POST_APPLICATION_CALLBACK, state = '*')
async def get_category_of_application_callback_handler(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    ...
    if not client_has_already_sent_app_to_check:
        kb = get_kb_with_categories(for_callback = GET_CATEGORY_OF_POST, 
            text_for_back_button = POST_APPLICATION_CALLBACK)
        await callback.message.edit_text('Выберите категорию вашей заявки:', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_CATEGORY_OF_POST), state = '*')
async def get_application_to_post(callback: CallbackQuery, state: FSMContext):
    category = int(callback.data.split(CALLBACK_SEP)[1])
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = POST_APPLICATION_CALLBACK))
...
async def send_new_application_to_admin(message: Message, state: FSMContext):
    category = int((await state.get_data())['category'])
    await state.finish()
    kb = InlineKeyboardMarkup()
    file_name = str(message.from_user.id)+' '+str(message.message_id)
    with open(
        APPLICATIONS_FROM_USERS_DIR+'/'+msg_categories[category]+'/'+file_name,
    'w') as file:
        file.write(message.text + '\n 👉 '+(await get_nick(message.from_user)))
    kb.row(
        InlineKeyboardButton(
            'Подтвердить', 
            callback_data = CALLBACK_SEP.join(
                [CONFIRM_CHECK_APPLICATION, str(message.message_id), str(category)])
        ),
        InlineKeyboardButton('Отменить', callback_data = CALLBACK_SEP.join(
            [DENY_CHECK_APPLICATION, file_name, str(category)])
        )
    )
    await callback.bot.send_message(
        message.from_user.id, 'Вы уверены, что хотите разместить эту заявку?', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(DENY_CHECK_APPLICATION), state = '*')
async def deny_send_application(callback: CallbackQuery, state: FSMContext):
    ...
    category = int(category)
    try:
        remove(APPLICATIONS_FROM_USERS_DIR+'/'+msg_categories[category]+'/'+file_name)
    except FileNotFoundError:
        return # user already has deleted his own application
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton('Отправить другую заявку', callback_data = POST_APPLICATION_CALLBACK))
    await callback.message.edit_text(cancel_check_text, reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(CONFIRM_CHECK_APPLICATION), state = '*')
async def check_new_application(callback: CallbackQuery, state: FSMContext):
    ...
    kb.row(
        InlineKeyboardButton(
            'Да', callback_data = CALLBACK_SEP.join(
                [ALLOW_APPLICATION, str(callback.from_user.id), message_id, category])
        ),
        InlineKeyboardButton(
            'Нет', callback_data = CALLBACK_SEP.join(
                [DECLINE_APPLICATION, str(callback.from_user.id), message_id, category])
        )
    )
    admin_id = callback.from_user.id if TESTING else EUGENIY_ID
    await callback.bot.send_message(admin_id, 'Запрос на размещение заявки по категории '
        f'<b>{message_category_names[int(category)]}</b>:', parse_mode = 'HTML')
    await callback.bot.forward_message(admin_id, callback.from_user.id, message_id, disable_notification=True)
    await callback.bot.send_message(admin_id, 'Разместить заявку?', reply_markup = kb, disable_notification=True)
    try:
        await callback.message.edit_text(final_text)
    except:
        return

# @dp.callback_query_handler(lambda callback: callback.data.startswith(ALLOW_APPLICATION), state = '*')
async def post_application(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        msg_path = \
            (NEW_MESSAGES_PATH if not TESTING else TEMP_MESSAGES_PATH)+'/'+msg_categories[category]+\
                '/'+file_name+' '+datetime.now().strftime('%m_%d_$H_%M')
        mkdir(msg_path)
        move(
            APPLICATIONS_FROM_USERS_DIR+'/'+msg_categories[category]+'/'+file_name,
            msg_path+'/'+TEXT_INFO
        ...
        text = f'Failed to save to send application from {user_id}-"{file_name}": '+str(e)
        ...
        await callback.bot.send_message(
            user_id, 'Поздравляем, ваша заявка была одобрена администраторами!', reply_to_message_id = message_id)
        await callback.message.edit_text(final_text)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(DECLINE_APPLICATION), state = '*')
async def ask_coment_about_application_refusal(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    ...
    try:
        ...
        text = f'Failed to remove application from {user_id}-"{file_name}": '+str(e)
        logging.critical(text, exc_info = True)
    else:
        await generate_message_to_send_comment_about_refusal(callback, user_id, source_message_id)

async def generate_message_to_send_comment_about_refusal(callback: CallbackQuery, to_user: int, to_message_id: int):
    kb = InlineKeyboardMarkup()
    for i in range(len(comments_about_refusal)):
        kb.add(InlineKeyboardButton(comments_about_refusal[i], callback_data=CALLBACK_SEP.join(
            [ATTACH_COMMENT_ABOUT_APP_REFUSAL, str(i), str(to_user), str(to_message_id)])))
    try:
        await callback.message.edit_text(TEXT_AFTER_RFUSAL_OF_CLIENTS_APPLICATION, reply_markup=kb)
    except (MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty):
        pass

# @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_BACK_TO_CHOOSING_COMMENT_ABOUT_REFUSAL), 
    state = '*')
async def gen_msg_to_send_comment_again(callback: CallbackQuery, state: FSMContext):
    ...
    await generate_message_to_send_comment_about_refusal(callback, int(to_user_id), int(to_message_id))

# @dp.callback_query_handler(lambda call: call.data.startswith(ATTACH_COMMENT_ABOUT_APP_REFUSAL), state = '*')
async def decline_application_from_client(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    _, comment_index, to_user, to_message_id = callback.data.split(CALLBACK_SEP)
    comment_index, to_user, to_message_id = [int(i) for i in (comment_index, to_user, to_message_id)]
    # not the last variant of comment (where admin have to write it's own comment)
    if comment_index != len(comments_about_refusal)-1: 
        await callback.bot.send_message(
            to_user, f'Ваше объявление '
            'отклонено. Причина отклонения: ' + comments_about_refusal[comment_index], 
            reply_to_message_id = to_message_id
        )
        try:
            await callback.message.edit_text('Заявка удалена. сообщение об этом отправлено клиенту.')
        except (MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty):
            pass
    else:
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton(
            BACK_BUTTON_TEXT, 
            callback_data=CALLBACK_SEP.join(
                [GET_BACK_TO_CHOOSING_COMMENT_ABOUT_REFUSAL, str(to_user), str(to_message_id)])
        ))
        try:
            await callback.message.edit_text('Отправьте комментарий, объясняющий причину отмены заявки: ', 
                reply_markup = kb)
        except (MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty):
            pass
        else:
...
async def ask_to_confirm_specifical_comment_about_refusal_of_application(message: Message, state: FSMContext):
    await state.update_data(message = message.text)
    kb = InlineKeyboardMarkup()
    kb.row(
        InlineKeyboardButton('Да', callback_data = CONFIRM_SENDING_COMMENT_ABOUT_REFUSAL),
        InlineKeyboardButton('Нет', callback_data = DECLINE_SENDING_COMMENT_ABOUT_REFUSAL)
    )
    await callback.bot.send_message(
...
# @dp.callback_query_handler(lambda callback: callback.data == DECLINE_SENDING_COMMENT_ABOUT_REFUSAL, 
    state = States.get_comment_about_refusal)
async def decline_to_send_comment_about_refusal_of_application(callback: CallbackQuery):
    try:
        await callback.message.edit_text('Отправьте новый комментарий')
    except (MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty):
        pass

# @dp.callback_query_handler(lambda callback: callback.data == CONFIRM_SENDING_COMMENT_ABOUT_REFUSAL, 
    state = States.get_comment_about_refusal)
async def send_specific_comment_about_application_refusal(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    await state.finish()
    await callback.bot.send_message(
        data['to_user'], f'Ваше объявление '
        'отклонено. Причина отклонения: ' + data['message'], 
...
# @dp.message_handler(lambda message: message.text == ADS, state='*')
async def buy_ad(message: Message, state: FSMContext):
    await state.finish()
    await message.answer('Вы можете узнать актуальные цены на рекламу в боте через https://t.me/+2Grf99Dpkf4zM2Zi')

# @dp.message_handler(lambda message: message.text == BUY_NEW_CATEGORY, state='*')
async def buy_new_category(message: Message, state: FSMContext):
    await state.finish()
    await message.answer('Если твоего направления нет в "Категории заявок", то можешь его заказать. '
...
# @dp.message_handler(lambda message: message.text == PARTNER_SHOWCASE, state='*')
async def partner_showcase(message: Message, state: FSMContext):
    await state.finish()
    kb = InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton('Vitamin', url='https://vitamin.tools/?ref2=542a6907-a190-4ab5-8dea-7726b6e8fc3e'))
    await message.answer(
        'Vitamin\n\n'
...
# @dp.message_handler(lambda message: message.text == REPORT_ERROR, state = '*')
async def report_error_handler(message: Message, state: FSMContext):
    if message.text in [NEW_ORDER, NEW_ORDER_FOR_NEWBIE, PERSONAL_CABINET, ADDITIONAL_OPTIONS, SUBSRIBES_BUTTON, TAB_HELPFUL_BUTTON, ADD_EXTERNAL_CHAT, BUY_NEW_CATEGORY, PARTNER_SHOWCASE, ADS, POST_APPLICATION]:
        await state.finish()
        try:
            await callback.bot.send_message(message.from_user.id, 'Операция отменена')
        except:
            pass
    ...
    await state.finish()
    await report_error(message.from_user.id, 0) # 0 is index in list with replies to reports
...
# @dp.callback_query_handler(lambda c: c.data.startswith(REPORT_ERROR_BUTTON+CALLBACK_SEP), state = '*')
...
async def message_handler(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton('Ответить', callback_data = REPLY_TO_USER+CALLBACK_SEP+str(message.from_user.id))
    )
    await callback.bot.send_message(message.from_user.id, 'Спасибо за ваше сообщение, в ближайшее время мы вам ответим')
    for id in ((DEVELOPER_ID, EUGENIY_ID) if not TESTING else (message.from_user.id,)):
        username = ''
        if message.from_user.username:
            username = f'@{message.from_user.username}'
        text = ( f'{username}\n' if username else '') + f'({message.from_user.id}) ' + \
            DESCIPTIONS_ABOUT_REPORT[report_type]
        await callback.bot.send_message(id, text)
        await callback.bot.forward_message(id, message.chat.id, message.message_id)
        await callback.bot.send_message(id, '_', reply_markup = kb)

# @dp.callback_query_handler(lambda callback: callback.data.startswith(REPLY_TO_USER), state = '*')
...
async def get_answer(message: Message, state: FSMContext):
    ...
    if to_admin:
        kb.add(
            InlineKeyboardButton('Ответить', callback_data = REPLY_TO_USER+CALLBACK_SEP+str(user_id))
        )
        for id in ((message.from_user.id,) if TESTING else (DEVELOPER_ID, EUGENIY_ID)):
            await callback.bot.forward_message(id, message.from_user.id, message.message_id)
            await callback.bot.send_message(id, '_', reply_markup = kb)
    else:
        kb.add(
            InlineKeyboardButton('Ответить', callback_data = REPLY_TO_ADMIN+CALLBACK_SEP+str(user_id))
        )
        try:
            await callback.bot.send_message(user_id, message.text, reply_markup = kb)
        except:
            await message.answer('Не удаётся отправить ответ пользователю')
            return
    await message.answer('Ответ отправлен')

# @dp.callback_query_handler(lambda callback: callback.data.startswith(REPLY_TO_ADMIN), state = '*')
