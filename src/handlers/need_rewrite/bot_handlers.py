def init_callback_query_handlers(dp: Dispatcher, _ym_client: ymClient):
    global ym_client
    ym_client = _ym_client
    bot = dp.bot
    ...
    async def choose_order_category(callback: CallbackQuery, state: FSMContext):
        ...
        kb.row(
            InlineKeyboardButton(
                'Да', callback_data = CALLBACK_SEP.join([USE_PROMOCODE_BUTTON, category, ''])),
            InlineKeyboardButton(
                'Нет', 
    ...
    async def choosing_period_handler(callback : CallbackQuery, state: FSMContext):
        ...
        if admin:
            await callback.answer('Вы не можете оплачивать подписки, '
                f'так как ваш аккаунт подключен к {await get_nick((await bot.get_chat_member(admin, admin)).user)}')
            return
        try:
    ...
    async def make_trial(callback: CallbackQuery, state: FSMContext):
        ...
        await bot.send_message(client_id, 'Главное меню', reply_markup = main_kb)
    ...
    async def process_payment(
        ...
        commission = 0
        try:
            commission = await process_referal(client_id, cost, bot)
        except Exception as e:
        ...
        if not had_paid_period_for_current_category:
            ...
            await bot.send_message(current_client.id, 'Главное меню', reply_markup = main_kb)
        save_to_statistics(
    ...
    async def get_promocode(callback: CallbackQuery, state: FSMContext):
        ...
        if not promocode:
            ...
            state = dp.current_state(user = callback.from_user.id, chat = callback.from_user.id)
            # if promocode is for all categories bot will use chosen category
    ...
    async def check_given_promocode(promocode: str, chosen_category: int, from_user: User):
        if not promocode_is_in_db(promocode):
            await bot.send_message(from_user.id, f'Промокод "{promocode}" не существует')
            return
        ...
        if not pcd_is_endless and pcd_has_been_used:
            await bot.send_message(from_user.id, f'Вы уже использовали промокод "{promocode}"')
            return
    ...
    async def process_using_trial_promocode(promocode: str, category: Union[int, None], trial_extra_days: int, client_id: int):
        ...
        if not Client.did_activate_bot(client):
            await bot.send_message(client.id, f'Не забудьте активировать бота @{bot_names[client.sending_mode]}')
    ...
    async def pause_period(message: Message, state: FSMContext):
        if not message.text.isdigit() or '-' in message.text:
            await bot.send_message(message.from_user.id, 'Не корректное значение, попробуйте ещё раз')
            return
        data = await state.get_data()
    ...
    async def get_tg_id_to_transfer(callback: CallbackQuery, state: FSMContext):
        ...
        if admin:
            try:
                nick = await get_nick(await bot.get_chat_member(admin, admin))
            except:
                nick = admin
    ...
    async def get_confirmation_to_transfer(message: Message, state: FSMContext):
        ...
        await state.finish()
        try:
            nick = await get_nick((await bot.get_chat_member(new_id, new_id)).user)
        except:
            nick = new_id
        kb = InlineKeyboardMarkup()
        kb.add(
    ...
    async def transfer_account(callback: CallbackQuery, state: FSMContext):
        ...
        kb.add(InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data = SUBSCRIBES_CALLBACK))
        try:
            nick = await get_nick((await bot.get_chat_member(old_id, old_id)).user)
        except:
            nick = new_id
        await callback.message.edit_text(f'Все данные с {nick} полностью перенесенны',
            parse_mode='HTML', reply_markup = kb)
        await bot.send_message(old_id, 
    ...
    def get_kb_for_helpful_tab():
        ...
        kb.add(InlineKeyboardButton('Статья с обзором бота', url= 'https://vk.com/@golubin_bot-obzor-servisa-golubin-bot'))
    ...
    @dp.callback_query_handler(lambda callback: callback.data.startswith(GET_INFO_ABOUT_BOT_FOR_NEWBIE), state = '*')
    ...
    async def get_category_to_send_last_messages(callback: CallbackQuery, state: FSMContext):
        ...
        if user.date_of_recieveing_latest_messages(user_id):
            await bot.send_message(user_id, 'Вы уже получили заявки')
            return
    ...
    async def expand_period(callback: CallbackQuery, state: FSMContext):
        ...
        commission = 0
        try:
            commission = await process_referal(callback.from_user.id, surcharge_amount, bot)
        except:
    ...
    async def try_to_find_client_to_expand_period(user: User, mode: int, days_to_expand: int) -> Client:
        try:
            ...
            await bot.send_message(DEVELOPER_ID, f'Failed to find {await get_nick(user)} after surcharging')
        return client
    ...
    async def show_all_stop_words(callback: CallbackQuery, state: FSMContext):
        ...
        if len(stop_words) > 50:
            await bot.send_document(client.id, InputFile(StopWords.get_path_to_filter(client), 'фильтр.txt'))
    ...
    async def get_stop_words_to_delete(message: Message, state: FSMContext):
        ...
        await bot.send_message(client.id, text)
    ...
    async def send_sale_for_review(callback: CallbackQuery, state: FSMContext):
        ...
        await bot.send_message(user_id, f'Ваш период по категории {message_category_names[category]} за отзыв расширен'
    ...
    async def check_new_application(callback: CallbackQuery, state: FSMContext):
        ...
        await bot.send_message(admin_id, 'Запрос на размещение заявки по категории '
            f'<b>{message_category_names[int(category)]}</b>:', parse_mode = 'HTML')
        await bot.forward_message(admin_id, callback.from_user.id, message_id, disable_notification=True)
        await bot.send_message(admin_id, 'Разместить заявку?', reply_markup = kb, disable_notification=True)
        try:
    ...
    async def post_application(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            await bot.forward_message(DEVELOPER_ID, user_id, message_id)
        else:
            await bot.send_message(
    ...
    async def message_handler(message: Message, state: FSMContext):
        ...
        await bot.send_message(message.from_user.id, 'Спасибо за ваше сообщение, в ближайшее время мы вам ответим')
        for id in ((DEVELOPER_ID, EUGENIY_ID) if not TESTING else (message.from_user.id,)):
            ...
            await bot.forward_message(id, message.chat.id, message.message_id)
            await bot.send_message(id, '_', reply_markup = kb)
    ...
    async def get_answer(message: Message, state: FSMContext):
        ...
        if to_admin:
            ...
            for id in ((message.from_user.id,) if TESTING else (DEVELOPER_ID, EUGENIY_ID)):
                await bot.forward_message(id, message.from_user.id, message.message_id)
                await bot.send_message(id, '_', reply_markup = kb)
        else:
            kb.add(
            ...
            try:
                await bot.send_message(user_id, message.text, reply_markup = kb)
            except:
