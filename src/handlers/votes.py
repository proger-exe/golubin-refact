...
from aiogram.utils.exceptions import MessageNotModified, MessageCantBeEdited, MessageTextIsEmpty
from aiogram.dispatcher import FSMContext
from aiogram.types import Message, CallbackQuery
from aiogram_calendar import SimpleCalendar, simple_cal_callback
from src.apis.ClientsData.StopWords import edit_bot_filter
from src.data import bot_config, config
from src.data.modules.votes import CONFIRM_SPAM, DISCARD_SPAM, GET_VOTE_STATISTICS, JUST_DEL_MSG, THROW_MSG_TO_SPAM
from src.apis.db.modules.votes import generate_statistics_texts_for_message
from src.utils.client import Client 
from src.data.bot_config import MESSAGE_FILTER, MIN_PERIOD_TO_GET_VOTE_BUTTONS, days_per_period
from src.utils.message_deleting import del_spam_message

handle_get_vote_period_for_statistics = lambda dp: dp.register_message_handler(get_vote_period_for_statistics, text=GET_VOTE_STATISTICS, state="*")
...
handle_set_period = lambda dp: dp.register_callback_query_handler(set_period, simple_cal_callback.filter(), state="get_period_for_votes_statistics")
...
handle_set_message_as_spam = lambda dp: dp.register_callback_query_handler(set_message_as_spam, text_startswith=CONFIRM_SPAM, state="*")
async def set_message_as_spam(call: CallbackQuery, state: FSMContext):
    ...
    current_bot = Bot(bot_config.bot_tokens[category])
...
handle_cancel_setting_message_as_spam = lambda dp: dp.register_callback_query_handler(cancel_setting_message_as_spam, text_startswith=DISCARD_SPAM, state="*")
async def cancel_setting_message_as_spam(callback: CallbackQuery, state: FSMContext):
    ...
    """
    await state.finish()
    _, to_user, orig_msg_id, category = callback.data.split(bot_config.CALLBACK_SEP)
