...
def save_to_statistics(
	...
	ref_link: Union[str, None] = None # if there ref_link is not null, the current statistics 
								      # will be uploaded for the referal or promocode as well
):
	conn, cursor = get_connection_and_cursor()
	today = datetime.now().strftime(DATE_FORMAT)
	cursor.execute(f'SELECT COUNT({NEW_CLIENTS}) FROM {STATISTICS_TABLE} WHERE {DATE} = "{today}"')
	if not cursor.fetchall()[0][0]:
		...
		conn.commit()
	if ref_link: 
		_save_referal_stats(conn, cursor, today, ref_link, bot_activates, new_users, new_clients, 
			new_payments, payments_sum, total_referal_commisions, repeated_payments, conversions_to_client)
...
def get_statistics_by_date(
	date: datetime, 
	fields = (NEW_CLIENTS, TOTAL_PAYMENT_SUM, PAYMENT_NUMBER, 
	NEW_USERS, REPEATED_PAYMENTS, CONVERSIONS_TO_CLIENT, ACTIVATED_TRIAL,
...
def get_statistics_by_period(
	date1: datetime, 
	date2: datetime,
	fields = (NEW_CLIENTS, TOTAL_PAYMENT_SUM, PAYMENT_NUMBER, NEW_USERS, 
	REPEATED_PAYMENTS, CONVERSIONS_TO_CLIENT, 
	ACTIVATED_TRIAL, WERE_GOT_LAST_MESSAGES)
) -> tuple:
...
def _fetch_statistics_result_and_close(cursor: Cursor, connection: Connection, fields_len: int) -> tuple:
...
def increase_new_messages_number(for_category: int, by_number: int = 1, for_time: datetime = None):
	now = for_time if for_time else datetime.now()
	now = now.replace(minute=0, second=0, microsecond=0)
	now = now.strftime(TIMESTAMP_FORMAT)
	conn, cursor = get_connection_and_cursor()
	cursor.execute(F'SELECT COUNT({CATEGORY}) FROM {NEW_MESSAGES_NUMBER_TABLE} WHERE {CATEGORY} = {for_category} AND '
		F'{TIME} = "{now}"')
	if cursor.fetchall()[0][0]:
		cursor.execute(f'UPDATE {NEW_MESSAGES_NUMBER_TABLE} SET {MSG_NUMBER} = {MSG_NUMBER} + {by_number} WHERE '
			F'{TIME} = "{now}" AND {CATEGORY} = {for_category}')
	else:
		cursor.execute(f'INSERT INTO {NEW_MESSAGES_NUMBER_TABLE} VALUES ("{now}", {for_category}, {by_number})')
	commit_and_close_connection_and_cursor(conn, cursor)

def get_new_messages_number(for_time: Union[date, datetime], for_category: int = None) -> int:
	is_datetime = False
	...
	if not is_datetime:
		time_filter = f'"{str_time}" <= {TIME}  AND {TIME} < "{str_time}" + INTERVAL 1 DAY'
	else:
	...
	conn, cursor = get_connection_and_cursor()
	cursor.execute(
		f'SELECT SUM({MSG_NUMBER}) FROM {NEW_MESSAGES_NUMBER_TABLE} WHERE {time_filter}' + category_filter)
	result = cursor.fetchall()
	close_connection_and_cursor(conn, cursor)
...
def get_average_messages_number(hour: int, period: int = -1, week_day: int = None, category: int = None) -> float: 
	''' 
	...
	conn, cursor = get_connection_and_cursor()
	cursor.execute(f'SELECT SUM({MSG_NUMBER})/COUNT(DISTINCT({TIME})) FROM {NEW_MESSAGES_NUMBER_TABLE} '
...
def calculate_conversions() -> typing.Tuple[float, float, float]:
	'''
	returns:
	...
	cursor.execute(f'SELECT COUNT(DISTINCT({ID_COL})) FROM {CLIENTS_SUBSCRIBES} AS client WHERE {IS_PAID} AND '
	...
	close_connection_and_cursor(conn, cursor)
	return num_of_users_that_tried_trial / users_number, number_of_clear_clients / users_number, \
		number_of_funnel_clients / users_number

def get_referal_statistics(referal_link: str, for_date: Union[date, None] = None
