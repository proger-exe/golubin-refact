...
from src.data.modules.promocodes import *
...
def add_promocode(
    ...
    if trial_days and sale_size:
        raise (
            ValueError(
                "Promocodes are not allowd to be for trial days and sale at the one time"
            )
        )
    if len(promocode) > PROMOCODE_MAX_LENGTH:
        raise (
            ValueError(
                f'Length of "{promocode}" is greater than max allowed: {PROMOCODE_MAX_LENGTH}'
            )
        )
...
def promocode_is_in_db(promocode: str, cursor: Cursor = None) -> bool:
    ...
    if not cursor:
        conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f'SELECT COUNT({PROMOCODE}) FROM {PROMOCODES_TABLE} WHERE {PROMOCODE} LIKE "{get_right_str(promocode)}"'
    )
...
def get_promocode_due_date(promocode: str) -> datetime:
    ...
    promocode = promocode.replace('"', '\\"')
    cursor.execute(
        f'SELECT {DUE_TIME} FROM {PROMOCODES_TABLE} WHERE {PROMOCODE} = "{promocode}"'
    )
    result = cursor.fetchall()
    close_connection_and_cursor(conn, cursor)
    if not result:
...
def get_promocode_refer(promocode: str) -> int:
    ...
    promocode = promocode.replace('"', '\\"')
    cursor.execute(
        f'SELECT {FROM_REFER} FROM {PROMOCODES_TABLE} WHERE {PROMOCODE} = "{promocode}"'
    )
    result = cursor.fetchall()
    close_connection_and_cursor(conn, cursor)
    if not result:
...
def get_last_activated_endless_promocode(
    client_id: int, for_category: int = None, for_period: int = None
) -> Union[str, None]:
    ...
    period_filter = (
        "" if for_period is None else f"AND {FOR_PERIOD} IN (NULL, {for_period})"
    )
    cursor.execute(
        f"SELECT {PROMOCODE} FROM {PROMOCODES_TABLE} AS pcd"
    ...
    if not all_eternal_pcds:
        ...
        return None
    cursor.execute(
        f"SELECT {USED_PROMOCODES} FROM {USED_PROMOCODES_TABLE} WHERE {CLIENT_ID} = {client_id}"
    )
...
def set_promocode_as_used_by_client(client_id: int, promocode: str):
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"SELECT COUNT({CLIENT_ID}) FROM {USED_PROMOCODES_TABLE} WHERE {CLIENT_ID} = {client_id}"
    )
    promocode = promocode.replace('"', '\\"')
    if cursor.fetchall()[0][0]:
        ...
        cursor.execute(
            f'INSERT INTO {USED_PROMOCODES_TABLE} VALUES({client_id}, ";{promocode};")'
        )
    commit_and_close_connection_and_cursor(conn, cursor)
...
def set_endless_sale_on_promocode(promocode: str):
    ...
    try:
        ...
        raise e
    cursor.execute(
        f'UPDATE {PROMOCODES_TABLE} SET {FOREVER} = 1 WHERE {PROMOCODE} = "{promocode}"'
    )
    commit_and_close_connection_and_cursor(conn, cursor)
...
def promocode_is_endless(promocode: str) -> bool:
    ...
    if not promocode_is_in_db(promocode, cursor):
        raise ValueError(f'There is not promocode "{promocode}" in database')
    cursor.execute(
        f'SELECT {FOREVER} FROM {PROMOCODES_TABLE} WHERE {PROMOCODE} = "{get_right_str(promocode)}"'
    )
    result = cursor.fetchall()
    close_connection_and_cursor(conn, cursor)
    return result[0][0]
...
def transfer_promocodes_info(from_id: int, to_id: int):
    ...
    cursor.execute(
        f"UPDATE {PROMOCODES_TABLE} SET {FROM_REFER} = {to_id} WHERE {FROM_REFER} = {from_id}"
    )
    cursor.execute(
        f"UPDATE {USED_PROMOCODES_TABLE} SET {CLIENT_ID} = {to_id} WHERE {CLIENT_ID} = {from_id}"
    )
    commit_and_close_connection_and_cursor(conn, cursor)
...
def get_id_of_promocode(promocode: str) -> int:
    ...
    promocode = promocode.replace('"', '\\"').replace("\\", "\\\\")
    cursor.execute(
        f'SELECT {ID_COL} FROM {PROMOCODES_TABLE} WHERE {PROMOCODE} = "{promocode}"'
    )
    result = cursor.fetchall()
    close_connection_and_cursor(conn, cursor)
    if not result:
...
def get_all_active_promocodes_of_refer(refer_id: int) -> List[str]:
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"SELECT {PROMOCODE} FROM {PROMOCODES_TABLE} WHERE {FROM_REFER}={refer_id} AND {DUE_TIME} > NOW()"
    )
    result = cursor.fetchall()
    close_connection_and_cursor(conn, cursor)
