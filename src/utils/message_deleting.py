...
def delete_message_kb(
    ...
    kb.add(
        bt_type(
            "Удалить",
            callback_data=DELETE_MESSAGE + CALLBACK_SEP + str(relative_msg_id),
        )
    )
    return kb
...
def get_index_for_new_message() -> int:
    ...
    # delete the last gag
    cursor.execute(
        f"DELETE FROM {SENDED_MESSAGES} WHERE {RELATIVE_MSG_ID} = {index-1} AND {BOT_ID} = -1"
    )
...
def save_sended_message_data(
    ...
    bot_id: int,
    inside_bot_msg_id: int = -1,
):
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"INSERT INTO {SENDED_MESSAGES} VALUES ({relative_message_id}, {inside_bot_msg_id}, {client_id}, "
        f"{sended_message_id}, {bot_id})"
    )
    commit_and_close_connection_and_cursor(conn, cursor)


def get_sended_message_data(
    relative_message_id: int, bot_id: int, user_id: int = None
) -> tuple:
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"SELECT {RECIEVER_ID}, {SENDED_MSG_ID} FROM {SENDED_MESSAGES} WHERE "
...
def get_relative_message_id_by_inside_bot_msg_id(
    inside_bot_msg_id: int, bot_id: int
) -> int:
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"SELECT DISTINCT({RELATIVE_MSG_ID}) FROM {SENDED_MESSAGES} WHERE "
...
def get_sended_message_data_by_inside_bot_msg_id(
    inside_bot_msg_id: int, bot_id: int
) -> tuple:
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
...
async def delete_sended_msg(
    bot: Bot, sended_msg_data: tuple, except_for: Tuple[int, int] = ()
) -> tuple:
    ...
    for client_id, msg_id in sended_msg_data:
        if (
            client_id,
            msg_id,
        ) == except_for or client_id in USERS_THAT_ALLOWED_TO_DELETE_MESSAGES:
            continue
        total += 1
        try:
            ...
            unsuccesfully += 1
            logging.error(
                f"Failed to delete message ({client_id}, {msg_id}): " + str(e),
                exc_info=True,
            )
            continue
        else:
...
async def del_sended_message_by_callback_query(
    call: CallbackQuery, bot: Bot, bot_id: int
):
    ...
    if not sended_msg_data:
        try:
            await bot.edit_message_text(
                "Сообщение уже было удалено ранее",
                call.from_user.id,
                call.message.message_id,
            )
        except:
            await bot.send_message(
                call.from_user.id, "Сообщение уже было удалено ранее"
            )
        return
    try:
        await bot.edit_message_text(
            "Сообщение удаляется, ждите...", call.from_user.id, call.message.message_id
        )
    except:
        await bot.send_message(call.from_user.id, "Сообщение удаляется, ждите...")
    await delete_message_and_get_info_about_it_to_admin(
        bot, bot_id, sended_msg_data, relative_message_index, call
    )
...
async def delete_message_and_get_info_about_it_to_admin(
    ...
    try:
        succesfully, unsuccesfully, total = await delete_sended_msg(
            bot, sended_msg_data, (call.from_user.id, call.message.message_id)
        )
    except Exception as e:
        logging.error(
        ...
        text = (
            f"Всего разослано: <b>{total}</b>\nУдалено: <b>{succesfully}</b>"
            f"\nНе удалось удалить: <b>{unsuccesfully}</b>"
        )
        try:
            await bot.edit_message_text(
                text, call.from_user.id, call.message.message_id, parse_mode="HTML"
            )
        except:
...
def delete_sended_message_from_temp(bot_id: int, relative_message_index: int):
    if bot_id != PAYING_BOT_ID:
        try:
            if not TESTING:
                try:
                    rmtree(
                        NEW_MESSAGES_PATH
                        + SENDED_DIR
                        + msg_categories[bot_id - SENDER_BOT_ID]
                        + f"/{relative_message_index}"
                    )
            ...
            logging.error(
                f"Failed to delete message`s directory for ({relative_message_index}): "
                + str(e),
                exc_info=True,
            )


def get_relative_message_id(
    bot_id: int, sent_msg_id: int, user_id: int
) -> Union[int, None]:
    conn, cursor = get_connection_and_cursor()
    cursor.execute(
        f"SELECT {RELATIVE_MSG_ID} FROM {SENDED_MESSAGES} WHERE {RECIEVER_ID} = {user_id} AND "
...
async def delete_message_from_inside_bot(
    inside_bot_message_id: int, category: int, bot: aiogram.Bot
):
    bot_id = SENDER_BOT_ID + category
    msg_data = get_sended_message_data_by_inside_bot_msg_id(
        inside_bot_message_id, bot_id
    )
    ...
    if not succesfully and total > 0:
        ...
        relative_message_id = get_relative_message_id_by_inside_bot_msg_id(
            inside_bot_message_id, bot_id
        )
        delete_saved_message_data(relative_message_id, bot_id)
