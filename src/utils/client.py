...
class Client:
    ...
    def __str__(self) -> str:
        return (
            f"{self.__id};{self.get_str_payment_date()};{self.get_str_payment_date_end()};"
    ...
    def warning_status(self, conn: MySQLConnection, cursor: MySQLCursor) -> int:
        cursor.execute(
            f"SELECT {PAYMENT_ENDING_WARNING_STATUS} FROM "
            f"{CLIENTS_SUBSCRIBES} WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if not result:
    ...
    @property
    @database_working
    def trial_start(self, conn: MySQLConnection, cursor: MySQLCursor) -> datetime:
        cursor.execute(
            f"SELECT {TRIAL_START} FROM {TRIAL_PERIOD_INFO_TABLE} "
    ...
    @property
    @database_working
    def trial_period_end(self, conn: MySQLConnection, cursor: MySQLCursor) -> datetime:
        cursor.execute(
            f"SELECT {TRIAL_PERIOD_END} FROM {TRIAL_PERIOD_INFO_TABLE} "
            f"WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if not result:
    ...
    @property
    @database_working
    def used_sale(self, conn: MySQLConnection, cursor: MySQLCursor) -> bool:
        cursor.execute(
            f"SELECT {USED_SALE} FROM {TRIAL_PERIOD_INFO_TABLE} "
            f"WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if not result:
    ...
    def is_asked_why_didnt_pay(
        ...
        cursor.execute(
            f"SELECT {IS_ASKED} FROM {TRIAL_PERIOD_INFO_TABLE} "
            f"WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if not result:
    ...
    def is_aware_about_referal(
        ...
        cursor.execute(
            f"SELECT {IS_AWARE_ABOUT_REFERAL} FROM "
    ...
    @property
    @database_working
    def max_pause_days(self, conn: MySQLConnection, cursor: MySQLCursor) -> int:
        self.create_pauses_info_if_doesnt_have(conn, cursor)
        cursor.execute(
            f"SELECT {MAX_PAUSE_DAYS} FROM {CLIENTS_PAUSES} "
    ...
    def create_pauses_info_if_doesnt_have(
        self, conn: MySQLConnection, cursor: MySQLCursor
    ):
        try:
            cursor.execute(
                f"SELECT COUNT({CLIENT_ID}) FROM {CLIENTS_PAUSES} WHERE {CLIENT_ID} = {self.__id} "
                f"AND {CATEGORY} = {self.__sending_mode}"
            )
            if not cursor.fetchall()[0][0]:
                cursor.execute(
                    f"INSERT INTO {CLIENTS_PAUSES} VALUES ({self.__id}, {self.__sending_mode}, 0, 0, NULL)"
                )
                conn.commit()
    ...
    @property
    @database_working
    def used_pause_days(self, conn: MySQLConnection, cursor: MySQLCursor) -> int:
        self.create_pauses_info_if_doesnt_have(conn, cursor)
        cursor.execute(
            f"SELECT {PAUSE_DAYS_USED} FROM {CLIENTS_PAUSES} "
            f"WHERE {CLIENT_ID} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if not result:
    ...
    def unpause_date(self, conn: MySQLConnection, cursor: MySQLCursor) -> datetime:
        self.create_pauses_info_if_doesnt_have(conn, cursor)
        cursor.execute(
            f"SELECT {UNPAUSE_DATE} FROM {CLIENTS_PAUSES} WHERE "
    ...
    def unpause_date(self, date: datetime):
        ...
        if date:
            set_str = (
                f'SET {UNPAUSE_DATE} = "{date.strftime(Client.__TIMESTAMP_FORMAT)}" '
            )
        else:
    ...
    @property
    @database_working
    def is_offered_sale_after_end_of_trial(
        self, conn: MySQLConnection, cursor: MySQLCursor
    ) -> bool:
        cursor.execute(
    ...
    def id_of_message_with_sale_after_end_of_trial(self, msg_id: int):
        conn, cursor = get_connection_and_cursor()
        cursor.execute(
            f"UPDATE {TRIAL_PERIOD_INFO_TABLE} SET {MSG_ID_WITH_SALE_AFTER_TRIAL_END} = {msg_id} "
            f"WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        commit_and_close_connection_and_cursor(conn, cursor)
    ...
    def payment_period(self) -> int:
        return (self.__payment_period_end - self.__last_payment_date).days
    ...
    @property
    @database_working
    def is_using_trial(self, conn: MySQLConnection, cursor: MySQLCursor) -> bool:
    ...
    def set_trial_period(self, trial_end: datetime):
        ...
        if not cursor.fetchall()[0][0]:
            cursor.execute(
                f"INSERT INTO {TRIAL_PERIOD_INFO_TABLE} "
    ...
    def was_offered_sale(self, conn: MySQLConnection, cursor: MySQLCursor) -> bool:
        cursor.execute(
            f"SELECT {WAS_OFFERED_SALE} FROM {TRIAL_PERIOD_INFO_TABLE} "
            f"WHERE {ID_COL} = {self.__id} AND {CATEGORY} = {self.__sending_mode}"
        )
        result = cursor.fetchall()
        if result:
    ...
    def was_offered_sale(self, val):
        conn, cursor = get_connection_and_cursor()
        cursor.execute(
            f"UPDATE {TRIAL_PERIOD_INFO_TABLE} SET "
    ...
    def did_activate_bot(
        client_entity, category: int = None
    ) -> bool:  # client_entity: Union[int, Client]
        conn, cursor = get_connection_and_cursor()
        id, category = Client.__get_id_and_category_from_client_entity(
            client_entity, category
        )
        cursor.execute(
            f"SELECT COUNT(*) FROM {ACTIVATED_BOT_TABLE} WHERE {ID_COL} = {id} AND {CATEGORY} = {category}"
        )
    ...
    def set_as_activated_bot(client_entity, category: int = None):
        ...
        if not Client.did_activate_bot(client_entity, category):
            cursor.execute(
                f"INSERT INTO {ACTIVATED_BOT_TABLE} VALUES({id}, {category})"
            )
            conn.commit()
        close_connection_and_cursor(conn, cursor)

    @staticmethod
    def __get_id_and_category_from_client_entity(
        client_entity, category: int
    ...
    def get_all_clients_from_db(
        conn: MySQLConnection, cursor: MySQLCursor
    ) -> typing.List[Client]:
    ...
    @staticmethod
    @database_static_working
    def get_all_paid_clients(
        connection: MySQLConnection, cursor: MySQLCursor
    ) -> typing.List[Client]:
        str_now = datetime.now().strftime(Client.__TIMESTAMP_FORMAT)
        cursor.execute(
            f"SELECT * FROM {CLIENTS_SUBSCRIBES} WHERE "
    ...
    def get_clients_by_filter(
        ...
        if payment_date != None:
            if not (payment_period_end != None and choose_between_dates):
                after.append(
                    Client.__generate_ejecting_request_row_for_date(
                        LAST_PAYMENT, payment_date, choose_between_dates, greater
                    )
                )
        ...
        if payment_date != None and payment_period_end != None and choose_between_dates:
            after.append(
                Client.__generate_ejecting_request_row_for_date(
                    LAST_PAYMENT, payment_date, choose_between_dates, greater=True
                )
            )
            after.append(
                Client.__generate_ejecting_request_row_for_date(
        ...
        if warning_status != -1:
            after.append(
                Client.__generate_ejecting_request_row(
        ...
        if payment_period != None:
            ...
            after.append(
                f"DATEDIFF({PAYMENT_PERIOD_END}, {LAST_PAYMENT}) {sign} {payment_period}"
            )
        if is_paid != None:
        ...
        if is_using_trial != None:
            trial_filter = " {0} EXISTS (SELECT {1} FROM {2} WHERE {3} > NOW() {4} AND ({1}, {5}) = (client.{1}, client.{5}))".format(
                "" if is_using_trial else "NOT",
                ID_COL,
                TRIAL_PERIOD_INFO_TABLE,
                TRIAL_PERIOD_END,
        ...
        if is_paused != None:
            ...
            pause_filter = (
                f" {belonging} EXISTS (SELECT {CLIENT_ID} FROM {CLIENTS_PAUSES} WHERE ({CLIENT_ID}, {CATEGORY}) = "
    ...
    @staticmethod
    def __generate_ejecting_request_row(
        column_name: str, value: Union[Iterable, object]
    ) -> str:
        requst_row = ""
    ...
    def __generate_ejecting_request_row_for_date(
        column_name: str,
        date: Union[Iterable, datetime],
        choose_between_dates: bool,
        greater: bool,
    ) -> str:
        request_row = ""
        if isinstance(date, Iterable):
            if choose_between_dates:
                request_row = (
                    f"({column_name} BETWEEN '{date[0].strftime(Client.__TIMESTAMP_FORMAT)}' "
            ...
            if greater != None:
                ...
                request_row = Client.__generate_ejecting_request_row(
    ...
    def has_client_id(id: int) -> bool:
        ...
        cursor.execute(
            f"SELECT COUNT({ID_COL}) FROM {CLIENTS_SUBSCRIBES} WHERE {ID_COL} = {id}"
        )
        result = cursor.fetchall()[0][0]
    ...
    def add_to_db(self, conn: MySQLConnection, cursor: MySQLCursor):
        ...
        cursor.execute(
            f"SELECT COUNT({ID_COL}) FROM {CLIENTS_SUBSCRIBES} WHERE {ID_COL} = {self.__id} "
            f"AND {CATEGORY} = {self.__sending_mode}"
        )
    ...
    def change_id_of_client(old_id: int, new_id: int):
        ...
        cursor.execute(
            f"DELETE FROM {TRIAL_PERIOD_INFO_TABLE} WHERE {ID_COL} = {new_id}"
        )
        cursor.execute(f"DELETE FROM {CLIENTS_PAUSES} WHERE {CLIENT_ID} = {new_id}")
        cursor.execute(
            f"UPDATE {CLIENTS_SUBSCRIBES} SET {ID_COL} = {new_id} WHERE {ID_COL} = {old_id} "
        )
        cursor.execute(
            f"UPDATE {TRIAL_PERIOD_INFO_TABLE} SET {ID_COL} = {new_id} WHERE {ID_COL} = {old_id} "
        )
        cursor.execute(
            f"UPDATE {CLIENTS_PAUSES} SET {CLIENT_ID} = {new_id} WHERE {CLIENT_ID} = {old_id} "
        )
        commit_and_close_connection_and_cursor(conn, cursor)
    ...
    def is_awared_about(self, what: str) -> bool:
        ...
        conn, cursor = get_connection_and_cursor()
        cursor.execute(
            f'SELECT * FROM {AWARED_CLIENTS} WHERE {ID_COL} = {self.__id} AND {WHAT_AWARED_ABOUT} = "{what}"'
        )
        res = cursor.fetchall()
        close_connection_and_cursor(conn, cursor)
        if not res:
    ...
    def set_as_awared_about(self, what: str):
        ...
        if not res:
            cursor.execute(
                f'INSERT INTO {AWARED_CLIENTS} VALUES ({self.__id}, "{what}")'
            )
        commit_and_close_connection_and_cursor(conn, cursor)
