...
main_kb.row(KeyboardButton(NEW_ORDER), KeyboardButton(TAB_HELPFUL_BUTTON))
main_kb.row(KeyboardButton(REPORT_ERROR), KeyboardButton(PARTNER_SHOWCASE))
main_kb.add(KeyboardButton(PERSONAL_CABINET), KeyboardButton(ADDITIONAL_OPTIONS))
...
personal_cabinet_kb.add(KeyboardButton(MAIN_MENU))

additional_options_kb = ReplyKeyboardMarkup(resize_keyboard=True)
additional_options_kb.row(
    KeyboardButton(POST_APPLICATION), KeyboardButton(ADD_EXTERNAL_CHAT)
)
additional_options_kb.row(KeyboardButton(BUY_NEW_CATEGORY), KeyboardButton(ADS))
additional_options_kb.row(KeyboardButton(MAIN_MENU))


def get_kb_with_categories(
    text_for_back_button: str = "",
    requested_from: str = "",
    ...
    for cat in message_categories:
        ...
        kb.add(
            InlineKeyboardButton(
                message_category_names[cat], callback_data=callback_data
            )
        )
    if requested_from and text_for_back_button:
        kb.add(
            InlineKeyboardButton(BACK_BUTTON_TEXT, callback_data=text_for_back_button)
        )
    return kb


# this keyboard is sended when a client has used a trial period but didn't payed new one after the end
def kb_with_answers_why_didnt_buy_period(
    category: int, kb_type: type = OldKbType, bt_type: type = OldButtonType
        ...
        kb.add(
            bt_type(
                ...
                callback_data=CALLBACK_SEP.join(
                    [GET_ANSWER_WHY_DIDNT_BUY, str(i), str(category)]
                ),
            )
        )
    return kb
...
kb_for_newbie.row(KeyboardButton(NEW_ORDER), KeyboardButton(REFERAL_SYSTEM_BUTTON))
kb_for_newbie.row(KeyboardButton(TAB_HELPFUL_BUTTON), KeyboardButton(POST_APPLICATION))
kb_for_newbie.row(KeyboardButton(REPORT_ERROR), KeyboardButton(BUY_NEW_CATEGORY))
bot_senders = {}
...
def get_period_choosing_keyboard(
    ...
    postfix = CALLBACK_SEP.join(
        [
            str(mode),
            str_sales,
            str(int(with_sale_during_trial)),
            referal_link,
            requested_from,
        ]
    )
        ...
        if attach_trial:
            kb.add(
                ButtonType(
                    ...
                    callback_data=CALLBACK_SEP.join(
                        [ACTIVATE_TRIAL, str(mode), referal_link]
                    ),
                )
            )
    if with_get_last_msgs_button:
        kb.add(
            ButtonType(
                "Получить последние сообщения за сутки",
                callback_data=GET_LAST_MSGS_PER_DAY + CALLBACK_SEP + str(mode),
            )
        )
    if requested_from:
        kb.add(ButtonType(BACK_BUTTON_TEXT, callback_data=requested_from))
    return kb
...
def generate_inline_text_of_period_cost(period: str, category: int, sales: dict) -> str:
    prefix = ""
    if period == ONE_WEEK:
        prefix = "1 нед."
...
def delete_float_point_if_is_not_fraction(num: Union[Decimal, float]) -> str:
    int_num = int(num)
    if int_num - num == 0:
        num = str(int_num)
    else:
    ...
    return ButtonType(
        ...
        callback_data=CALLBACK_SEP.join(
            [
                ...
                str(int(with_sales_during_trial)),
                referal_link,
                requested_from,
            ]
        ),
...
def client_is_in_sale_period(client: Client):
    ...
    if not offering_date:
        return False
    return (
        (datetime.now() - offering_date).days <= SALE_PERIOD
        and not client.used_sale
        and client.was_offered_sale
    )


async def send_latest_messages(
    client: Client, for_period: timedelta = timedelta(days=1)
):
    for_period = min(for_period, timedelta(days=1))
    path_to_message = (
        (NEW_MESSAGES_PATH + SENDED_DIR + msg_categories[client.sending_mode])
        if not TESTING
        else (TEMP_MESSAGES_PATH + SENDED_DIR + msg_categories[client.sending_mode])
    )
    now = datetime.now()
    for m in listdir(path_to_message):
        ...
        if TEXT_INFO in files:
            ...
            try:
                ...
                continue
            try:
                await bot_senders[client.sending_mode].send_message(
                    client.id, text, reply_markup=kb, parse_mode="HTML"
                )
            ...
            try:
                await bot_senders[client.sending_mode].forward_message(
                    client.id, chat_id, message_id
                )
                if kb:
                    await bot_senders[client.sending_mode].send_message(
                        client.id, "_", reply_markup=kb
                    )
            except (ChatNotFound, BotBlocked):
                continue
            except Exception as e:
                logging.error(
                    f"Failed to forward message ({chat_id}, {message_id}) to new client: "
                    + str(e),
                    exc_info=True,
                )
...
async def try_to_send_latest_messages(
    client: Client, for_period: timedelta = timedelta(days=1)
):
    print("STARTED TO SEND LATEST MESSAGES")
    try:
        await bot_senders[client.sending_mode].send_message(
            client.id, "Сейчас Вам придут заявки за последние сутки"
        )
    except (ChatNotFound, BotBlocked):
    ...
    try:
        ...
        logging.error(
            "Failed to send latest messages to new user: " + str(e), exc_info=True
        )
...
def generate_text_and_keboard_for_payment(
    ...
    inline_kb_type: type = InlineKeyboardMarkup,
    inline_button_type: type = InlineKeyboardButton,
    ...
    h = blake2b(
        (str(period) + str(datetime.now()) + str(client_id) + str(cost)).encode(
            "utf-8"
        ),
        digest_size=10,
    )
    ...
    paying_buttons.append(
        inline_button_type(
            "Оплатить",
            url=get_yoomoney_payment_link(
                h.hexdigest(),
                period,
                ym_user.account,
                mode,
                cost if cost < YOOMONEY_TRANSFER_CAP else cost / Decimal("2"),
            ),
        )
    )
    str_mode = str(mode)
    ...
    callback_data = CALLBACK_SEP.join(
        [
            PAY_FOR_PERIOD_PREFFIX,
            period,
            h.hexdigest(),
            str_mode,
            str_with_sale_during_trial,
            referal_link,
            "1" if cost < YOOMONEY_TRANSFER_CAP else "2",
        ]
    )  # how many trasnfers an user must commit
    paying_buttons.append(inline_button_type("Оплатил ✅", callback_data=callback_data))
    kb.row(*paying_buttons)
    referal_payment_text = generate_text_about_oportunity_of_paying_from_ref_bal(
        client_id, cost
    )
    str_cost = delete_float_point_if_is_not_fraction(cost)
    ...
    try:
        ...
        kb.add(
            inline_button_type(
                "Оплатить с реферального счёта", callback_data=referal_callback_data
            )
        )
    except:
        logging.error(
    ...
    if with_back_button:
        kb.add(
            get_back_to_period_choosing(
                mode, sales, with_sale_during_trial, referal_link, requested_from
            )
        )
    ...
    str_cost = (
        f"<b>{str_cost}</b>"
        if str_cost == cost_without_sales
        else f"<s>{cost_without_sales}</s> <b>{str_cost}</b>"
    )
    return (
        True,
        f"Вы выбрали тариф на <b>{days_per_period[period]}</b> "
        f"дней (категория <b>{message_category_names[mode]}</b>), "
        f"стоимость: {str_cost} руб."
        + referal_payment_text
        + (
            "\nВ подарок консультация по продажам с создателем бота"
            if days_per_period[period] > days_per_period[SIX_MONTHS]
            else ""
        )
        + (
            ""
            if cost < YOOMONEY_TRANSFER_CAP
...
def get_yoomoney_payment_link(
    ...
    quickpay = Quickpay(
        receiver=ym_user_id,
        quickpay_form="shop",
        targets=f"Оплата работы Golubin бота на {days_per_period[payment_period]} дней "
...
def generate_text_about_oportunity_of_paying_from_ref_bal(
    user_id: int, cost: Decimal
) -> str:
